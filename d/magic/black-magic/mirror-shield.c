// This is a spell object. type -> other (defense electric).
// level    0    1    2    3    4        (reverse   energy)
// cost    30   45   60   75   90
// dec     10   20   35   55   60
// rev_dec  5    7   10   20    0
...
int cast(int level)
{
	...
	if( (int)me->query_skill(TYPE) < query_need_skill(level) ) {
	  tell_object( me, "你的"+to_chinese(TYPE)+"技能不足以使用這等級的魔法! \n"
      );
      return 0;
    }
	
    if( me->query_temp("effect/mirror-shield") ) {
      tell_object( me, "你上一個魔法鏡盾還沒消失呢! 別浪費法力了。\n" );
      return 0;
    }
	write( "你施展明鏡之盾咒文，在你的面前逐漸形成一層無形的力場。\n" );
	tell_room( environment(me), 
		me->query("c_name") + "喃喃地念了一段咒文，在"+
		me->query("c_name")+"的面前逐漸形成一層無形的力場。\n",	me );
	gain_spell_experience(me, TYPE, query_gain_spell_exp(level));
	me->set_temp("effect/mirror-shield", 1);
    me->apply_special_defense("electric", query_proportion(level) );
    me->apply_special_defense("energy", -query_reverse_proportion(level) );
...
{
    if( !caster ) return;
    tell_object( caster , "你感覺到你的魔法盾似乎消失了。\n" );
    caster->apply_special_defense("electric", -query_proportion(level) );
    caster->apply_special_defense("energy", query_reverse_proportion(level) );
    caster->set_temp("effect/mirror-shield", 0);
}
