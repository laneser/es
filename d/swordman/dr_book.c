...
#include <compress_obj.h>
	set_name("The book of DragonSlayer", "屠龍劍譜");
	add( "id", ({ "book" }) );
	set("unit","本");
	set_short("屠龍劍譜");
	set_long( @LONG
這是屠龍劍派的基本入門心法，其中記載了一些基本劍術和煉氣法門，
如果你想知道它的用法，可以用 help book 。
LONG
	);

	set( "weight", 10 );
	set( "extra_look", "$N帶著一本練劍養氣的「屠龍劍譜」。\n");
}
...
{
	if( !arg ) return notify_fail("你想對其它同門說什麼？\n");
