...
void create()
{
	...
	::create();
	set_level(16);
	set_name( "blessing monk", "藥王殿 祈福僧" );
	add( "id", ({ "blesser","monk","blessing" }) );
	set_short( "藥王殿 祈福僧" );
	set_long(@C_LONG
你看到一個臉色和善的僧人，他不像一般武僧那麼好鬥，他的任務是
協助他的同門作戰。
C_LONG
	);
	set("unit", "名" );
	...
	set( "chat_chance", 10 );
	set( "att_chat_output", ({
		"祈福僧說道: 你們還是放棄吧，你們撐不住的的 !\n",
		"祈福僧說道: 你們只要乖乖束手就縛我們絕不會為難你們的!\n",
        "祈福僧合什道：阿彌陀佛! 施主行為如此不堪，莫怪小僧無禮了!!\n"
    }) );
...
int my_tactic()
{
	...
	tell_room( environment(this_object()), 
		"祈福僧祈求道: 藥王菩薩 ! 請保護少林寺的使徒啊 ! .....\n"
		"\n天空灑下一團團銀白色的光芒，罩在少林僧侶們的身上。\n\n"
		, this_object()
	);
