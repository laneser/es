...
void kill_reward(object killer)
{
  ...
  if(explode(base_name(this_object()),"/")[0]== "u")
      if(!wizardp(killer))
       log_file("CHECK",killer->query("c_name")+" 殺死"+query("name")+"["+
       base_name(this_object())+"] 得到 "+exp+" 經驗"+" ["+extract(ctime(time()),4,15)+"]\n");
        return;
}
		...
		if( query("aggressive") && !victim->query("ghost") )
			...
			if( random(100) > (int)victim->query_stat("kar") ) {
					tell_object(victim, ( (tmp=query("c_killer_msg")) ?
						sprintf("%s\n", tmp):
						sprintf("突然，你發現 %s 正不懷好意的往你這裡衝過來....\n", query("short")) ) );
				kill_ob(victim);
			} else {
				tell_object(victim,
					sprintf("你感覺 %s 似乎正不懷好意的看著你...\n", query("short")));
			}
	}

	if( query("pursuing") && sscanf(str,"%s(%s)往%s邊離開。", tmp,name, direction)==3) {
		name = lower_case(name);
