...
#define C_ILLUSIONSA ({ "一股真氣由你的氣海穴流出, 緩緩的流向背部的陽堂穴...\n",\
	"一縷細微的真氣經由督脈流向足三里穴, 卻似忽遇到了阻礙而無法向前催動...\n",\
	...
	if( cond_data[1] > cond_data[3] ) {
		...
		if( cond_data[2]%4==0 ) {
			...
			message( "illusion", 
				...
				"你體內的氣運行到了第" + chinese_number(cond_data[2]/4) + "個大周天。\n"
				, player );
			if( cond_data[2] > 20 ) tell_room( environment(player), 
				"你看到一縷縷白氣從" + player->query("c_name") + "頂上冒出 ....。\n",
				player );
		} else {
			heal = force_effect;
			message( "illusion", 
...
//		player->delete_temp("block_message");
		...
		tell_object( player, 
			"你深深地吸了一口氣, 將周身運行的內息收回丹田, 站了起來。\n" );
		tell_room( environment(player), 
			player->query("c_name")+ "深深地吸了一口氣，站了起來。\n",
			player );
