...
int do_feast()
{
		...
		!inv[i]->query("user_corpse")) {
                        write(sprintf("你把%s抓起來，希哩呼嚕吃個乾淨 !!\n",
                                      inv[i]->query("short")));
                        tell_room(environment(),sprintf("%s歡呼一聲，把%s抓了起來，兩三下啃了個乾淨 ... 惡 ...\n",
        ...
        write("你擦擦嘴，拍拍肚子，滿足地打了個飽嗝。\n");
        tell_room(environment(),sprintf("%s擦擦嘴，拍拍肚子，滿足地打了個飽嗝。\n",this_object()->query("c_name")),this_object());
        return 1;
}
