...
void create()
{
        ...
        set_short( ""+HIW"銀塔巫師 創世者的跑腿"+NOR"" );
         set_long(@DESC
    魔導士拉修帝是很老但十分有經驗的魔法師。單由他的外表你幾乎無法想
像他到底有幾歲！現在他是魔法師公會的老大，他很樂意幫助年輕的魔法師，
你可以詢問他有關魔法師公會的一些資訊(guild,spells)。
DESC
        );
        set( "alignment", 2500 );
    set( "gender", "male" );
...
{
    tell_room( environment(this_object()), 
    sprintf("拉修帝很不屑的向%s說: 你還敢進來，活的不耐煩了，我就成全你，死吧!\n",who->query("c_name"))
    ,  ({ this_object(), who }) );
      "拉修帝很不屑的向你說: 你還敢進來，活的不耐煩了，我就成全你，死吧!\n"
    );
    return 1;
}
...
{//  50% call summoned monster -> 60% type1 40% type2 and stop.
 ...
 //  else 30% kick victim out! damage = 25-30.
    ...
    for( i=0; i<sizeof(mages); i++ ) {
      ...
      tell_object( victim,
        sprintf("%s跑過來幫助拉修帝攻擊你。\n",mages[i]->query("c_name")));
    }
tell_object( mages[i], 
        sprintf("你跑過去幫助拉修帝攻擊%s。\n",victim->query("c_name")));
...
t()->receive_healing(heal_point);
    ...
    if( (int)victim->query_level()<11 ||
      ...
      tell_object( victim, "拉修帝一腳把你踹了出去。\n" );
      tell_room( environment(this_object()), 
        sprintf("拉修帝一腳把%s踹了出去。\n",victim->query("c_name")),
        victim);
...
int hunt_check( object rashudi, object victim )
{
    ...
    if( victim && !member_group( victim, attackers ) &&
        ...
        sprintf("拉修帝大笑著說: %s，你以為 quit 就能逃出我的手掌心嗎? 死吧!\n",victim->query("c_name")
        , rashud
      )
      kill_ob( victim )
    return 1
...
int to_join(
    ...
    if( race == "lizardman" || race == "hawkman" || race == "orc" ) 
      tell_object( this_player()
        "拉修帝看了看你，搖搖頭說：回去吧！你不可能成為魔法師的。\n")
      return 1
    
    return 0
void ask_mage( object who 
    tell_object( who,@MSG
拉修帝說: 歡迎加入魔法師公會，除了蜥蜴人、半獸人以及鳥人之外�
          不論什麼種族，只要對魔法有興趣，都能夠成為我們的一員�
MSG
       if( hunt_check(this_object(), obj) ) return
ct( who
      "拉修帝說: 你可以到樓上的魔法圖書館去查閱你所要學的咒文。\n")
...
void warning1(object obj
     ...
     tell_object( obj
       "拉修帝狠狠地瞪著你！好像對你十分不爽！似乎準備要殺了你!\n" )
     tell_room( environment( obj ),
       sprintf("拉修帝狠狠地瞪著%s！好像對他十分不爽！\n" ,obj->query("c_name")), ob
     )
