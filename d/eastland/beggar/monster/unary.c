...
void create()
{
    ::create();
    set_level(16);
    set_name( "daughter unary", "月讀" );
    add( "id",({"daughter","unary"}) );
    set_short( "全副幫主的女兒 月讀");
    set_long(@LONG
月讀看起來約莫十五、六歲年紀,劍眉鳳眼,雙眉中略帶一絲英氣,平時更自比『花
木蘭』之流,頗有巾國女英雄之貌。她生平嫉惡如仇,路見不平往往拔刀相助,所以
你最好不要做壞事喔, 被她纏上可是無法善罷干休的。
LONG
            );
    set("unit","位");
    set("race","human");      
...
void do_kick(string arg)
{
    if (arg=="unary"){
       tell_object( this_player(),
          "\n月讀憤怨地向你說道:你這該死的小子去死吧 !!\n\n");
       tell_room( environment(this_object()),
          "\n月讀似乎對"+this_player()->query("c_name")+
          "很不爽並對他採取攻擊!!\n\n",this_player());
       this_object()->kill_ob(this_player());
       this_player()->block_attack(8);
       this_player()->set_temp("msg_stop_attack","(  你太失禮了,因而被攔住了動彈不得 !!  )\n");
       return ;
    }
}
...
{
     tell_object( asker,
       "月讀說道:咦,你問我爹爹喔?我不知道他在那裡耶。\n"
       "月讀嘆道:唉,爹爹不知道從什麼地方打聽到我祖父的下落,就整個人心神不寧,\n"
       "         一直嚷嚷著要見我祖父。這幾天不知道從那裡來了一個奇怪的人,自\n"
       "         他來了之後,爹爹就不常理我了。\n"
       "月讀說道:對了,你可以去找項長老,他和我爹最熟了。\n"
       );
