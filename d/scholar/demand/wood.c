...
int summon_me(object me)
{
        if ( (int)me->query_skill("polemic")< LEVEL*5 ) {
          write("你的辯術技能不夠驅使木精靈來幫助你。\n");
          return 1;
        }

        if ( (int)me->query_skill("yinyanglogy")< LEVEL*3 ) {
          write("你的陰陽學技能不夠驅使木精靈來幫助你。\n");
          return 1;
        }
	...
	if ( (int)me->query("spell_points") < COST_SP ) {
	  ...
	  return 1;
        ...
        tell_object(me,set_color("你集中精神開始利用心靈力量召喚木精靈來幫助你。\n","HIG"));
...
void check_battle(object me,object mob)
{
  if (!me->query_attacker()) {
    write("木精靈說：我的朋友、我必須要走了，假如須要我幫忙請再叫我。\n"); 
    tell_room(environment(me),set_color("你見到木精靈跳回樹洞，樹又縮了回去。\n","HIG"));
    remove_effect(me);
