// available function 
// 	void set_duration( int ) ;	效果長短
// 	void set_effect( int ) ;	( 1 - 25 )
//	void set_type( string );	種類( simple_poison || slow || weak )
//	void set_difficulty( int ); 	失敗率 ( 1 - 100 )
//	void set_time( int ); 		可用次數 ( 1 - ? )
// form 2 
...
{
	int rate,level,iq,dex,skill,kar,ali;
	skill = (int)me->query_skill("venomlogy");
	ali = (int)me->query("alignment") / (-10000);
	if ( skill == 0 ) 
		return 1;
...
// 60 % skill , 18% level, 14% dex 19%kar, total 111, alignment is bonus
	rate = skill*3/5 + dex + kar + level + iq + ali;
	if( random(rate) > diff ) 
		return 0;
	else
		return 1;
...
/*
	if ( times == 0 ) { // 應該不可能走到這裡
		write( "這" + query("unit") + query("c_name") + 
			"已經用完了，你隨手把它丟掉。\n" );
		remove();
		return 1;
	}
*/
	...
	if ( times == 0 ) { 
		write( sprintf("這%s已經用完了，你隨手把它丟掉。\n",query("unit"),query("c_name")) );
		remove();
	}
	return 1;
}
