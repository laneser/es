inherit WEAPON;

void create()
{
    set_name( "icy","冰魄" );
    set_short( "冰魄" );
    set_long(@C_LONG
這是飄雪仙姬心愛的玩具，乍看只是一枚萬年寒冰，可一旦遇上危險，
仙姬就會把它幻化成強力的攻擊武器。
C_LONG
            );
    set("unit","枚");
    set("weapon_class",50);
    set("type","longblade");
    set("max_damage",5);
    set("min_damage",0);
    set("weight",1);
    set("prevent_drop",1);
    set("value",({1,"silver"}) );
    set( "hit_func", "weapon_hit" );
    set("wield_func","daemon_wield");
    set("unwield_func","daemon_unwield");
}

void daemon_wield()
{
    if( (string)this_player()->query("name")=="snow") {
    set("prevent_drop",1);
    set("min_damage",30);
    set("max_damage",100);
    set("extra_look","@@color");
    tell_object( this_player(),set_color(
       "\n寒光一閃，『冰魄寒光劍』已出現在你手上．\n\n"
       ,"HIW") );
    tell_room( environment(this_player()),set_color(
       "\n"+"突然捲起一陣寒風，"+this_player()->query("c_name")+
       "手中已多了一把寒光閃閃的冰劍．\n\n"
       ,"HIW"),this_player() );
    set_name("icy sword","冰魄寒光劍");
    add("id",({"icy","sword"}) );
    set_short("冰魄寒光劍");
    return;
    }
}

void daemon_unwield()
{
        object holder ;
        if ( !holder = environment(this_object()) ) return;
    if( (int)this_object()->query("wc_damaged") ){
         tell_object( this_player(),"突然間強光一閃,你手中的冰魄寒光劍就消失不見了....\n");
         tell_room( environment(this_player()),
           "突然吹過一陣寒風"+this_player()->query("c_name")+"手中的冰魄寒光劍就消失了....\n",
           this_player() );
           this_object()->remove();
           return;
           }
    else{
    set("min_damage",0);
    set("max_damage",5);
    set("prevent_drop",1);
    delete("extra_look");
    set_name("icy","冰魄");
    set_short("冰魄");
    return ;
    }
}

int weapon_hit( object victim, int damage )
{
   object owner;
   int inte, intv, fp;

   owner = environment();
   inte = owner->query_stat("int");
   fp = owner->query("force_points");
   if (!((string)owner->query("name")=="snow")){
     tell_object( owner,
     "\n你手中的冰劍發出一陣清脆的聲音：哈哈 ! 你不配做我的主人。\n\n");
     owner->receive_special_damage("energy",inte/2);
     return 0;
   }
   if (!victim) return 0;
   intv = victim->query_stat("int");
   if( random(inte*3)>(intv*2) && fp >2 ) {
       tell_object( owner,
       "\n你將一陣內力貫輸於冰魄寒光劍, 劍突然發出一陣寒氣向"+victim->query("c_name"
)+"射去\n\n" );
       tell_object( victim, "\n" +
          owner->query("c_name") + "的寒光劍突然發出一陣寒氣向你襲來!\n\n" );
       tell_room( environment(owner),
     "\n" +
         owner->query("c_name") + "的冰劍突然向" + victim->query("c_name") + "射
出一陣寒氣!\n\n"
         , ({ victim, owner }) );
       victim->receive_damage(1000);
       victim->receive_special_damage( "energy", inte/2 );
          owner->add( "force_points", -3 );
          return inte/2;
        }
}



string color()
{
    return set_color("$N的右手中握著一柄萬年冰魄形成的白色長劍...\n","hiw");
}
