...
//	if(!ob) ob = previous_object();
	...
	if( !env ) { 
		write("你現在身在虛無飄渺間，不知何去何從。\n");
		return 1;
	}
	
	exits = env->query("exits");
	if( !mapp(exits) || !dirs = keys(exits) )
		return notify_fail("這裡沒有任何出路。\n");

	if( ob->query("confused") || ob->query_temp("confused") ) {
		tell_object( ob,"你的腦中一團混亂, 無法確實的控制身體!!\n");
	...
	if( (string)ob->query_temp("leader") &&
		...
		for( i=0; i<sz; i++ ) {
            ...
            if( environment(obs[i])!=environment(ob) ) {
                  tell_object(obs[i], "你跟你的隊伍走散了.\n");
  	              tell_object(ob,
                    sprintf("你的隊友 : %s 沒跟來.\n",obs[i]->query("c_name")) );
 			} else flag++;
		}
		if( sz > 1 && flag== sz ) write("你的隊友跟著你走過來.\n");
	}
