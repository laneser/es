...
void create()
{
	...
	::create();
	set_level(1);
	set_name( "Spirit of dwarf Prince Falady", "矮人王子法拉第的靈魂" );
	add ("id", ({ "prince", "falady", }) );
	set_short( "Spirit of Dwarf Prince Falady", "矮人王子法拉第的靈魂" );
	set_long(
    "a spirit of falady .\n",
	"法拉第的靈魂.\n"
	);
   ...
   if ( box->query("quest_item/queen_amulet") ) {
     tell_object(player,
      ...
      tell_room(environment(player),
        "法拉第被"+player->query("name")+"給捉進了瓶子.\n",
        player );
     box->set("quest_action/bottle_lock","falady");   
     }
   else {
     tell_object(player,"法拉第為了逃避你的魔法而消失了.\n");
     tell_room(environment(player),
     "法拉第為了逃避"+player->query("name")+"的魔法而消失了.\n",player);
     }  
   return ;
}
