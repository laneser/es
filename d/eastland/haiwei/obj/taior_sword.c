...
void create()
{
	set_name( "Sword of Taior", "太阿之劍" );
	add( "id", ({ "taior", "sword" }) );
	set_short( "太阿之劍" );
	set_long(
	"這是一把有著異樣深藍色劍刃漾著寒光的古劍，劍柄上刻著四個篆字「太阿神劍」。\n" 
	);
	set( "unit", "把" );
	set( "type", "longblade" );
...
int weapon_hit( object victim, int damage )
{
	...
	if( random(intelligence)>=15 && sp > 20 ) {
		tell_object( owner, 
			"\n你的太阿之劍突然發出一道青紫色的弧光，劍身變成明鏡一般的銀色！\n\n"
			);
		tell_room( environment(owner), 
			"\n" + owner->query("c_name") + "的太阿之劍突然發出一道青紫色的弧光，劍身變成明鏡一般的銀色！\n\n",
			owner );
