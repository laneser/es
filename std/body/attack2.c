...
int receive_damage(int damage)
{
	...
	if( damage <= 0 || link_data("dead") || hits < 1 || ob_data["linkdead"] )
		return 0;

	hits -= damage;
	if( hits < 1 && wizardp(this_object()) && ob_data["immortal"] )
		hits = (int)ob_data["max_hp"];
...
int calc_armor_class()
{
	...
	ac = ob_data["natural_armor_class"];
	db = ob_data["natural_defense_bonus"];
...
void calc_weapon_class()
{
	...
	wc1 += (int)ob_data["natural_weapon_class1"];
	wc2 += (int)ob_data["natural_weapon_class2"];
        wc1 += (int)tmp_ob_data["extra_wc"];
    wc1 -= ob_data["blind"];
	wc2 -= ob_data["blind"];
	if (wc1 < 1 ) wc1 = 1;
	if (wc2 < 1 ) wc2 = 1;
	min_dam1 += (int)ob_data["natural_min_damage1"];
	min_dam2 += (int)ob_data["natural_min_damage2"];
	max_dam1 += (int)ob_data["natural_max_damage1"];
	max_dam2 += (int)ob_data["natural_max_damage2"];
...
void unblock_attack()
{
	...
	i = ob_data["stop_attack"]-1;
	if( i > -1 ) ob_data["stop_attack"] = i;
...
int kill_ob(object victim)
{
	...
	if( !wizardp(this_object()) && ob_data["player_invisible"] ) {
		...
		tell_room(environment(),sprintf("你的前方一陣波動，%s的身影突然浮現出來。\n",ob_data["c_name"]),
			  this_object());
	}
	attackers += ({ victim });
	any_attack++;
...
void continue_attack()
{
	...
	if( ob_data["hit_points"] < 1 && !ob_data["ghost"] ) {
		die();
		return;
	}
	...
	// making an attack.
	if( ob_data["stop_attack"] > 0 ) {
                if( (msg = tmp_ob_data["msg_stop_attack"]) )
                        write( msg );
                else write (
                        "( 你還沒有完成上一個動作，失去一次攻擊機會 )\n");
		return;
	}
