...
varargs string finger_user(string who, int st)
{
	...
	if( !link->query("last_on") )
        msg += (tmp1 ? "上線時間:\t" : "上次上線時間:\t") + "<不明>\n";
	else
        msg += ((tmp1 && !tmp1->query("npc")) ? "上線時間:\t" : "上次上線時間:\t") +
        ctime((int)link->query("last_on"));
    if(st==1)
        msg += " (來自 " + (string)link->query("ip") + " )\n";
    else
        msg += "\n";
//	if (tmp1) {
...
//	}
    ...
    if( player&&tmp1 ) {
      age = (int)player->query_age();
        msg += sprintf("年齡:\t\t%d天 %d小時 %d分 %d秒\n",
           age/86400, (age%86400)/3600, (age%3600)/60, age%60 );
 
/* This section comment out by Elon, 12-23-94. 轉世, 陽壽制度還沒好
...
"=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n";
    if (j == 0)
        msg += "[ 東方故事現在沒有任何人上線中, 那你一定不是人 ]\n";
	else {
        msg += "[ 東方故事 ] 目前有 " + j + " 位使用者 ( 時間: "+
               ctime(time()) + " )\n";
		msg += 
"=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n";
