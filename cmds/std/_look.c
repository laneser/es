...
//			sprintf("%s仔細的打量著%s。\n", user->query("c_name"), (living(it)? foo: lower_case(foo))) }),
//			({ user, it }) );
	}
...
//Looks like there isn't any light here.
  return (if_here?"這":"那")+"裡一片漆黑，伸手不見五指。\n";
}
...
//	name = contents[i]->query("name");
	...
	if (count > 1) {
		long = sprintf("%s  %s%s%s(%s).\n",long,convert_c_number(count),
						(obj->query("unit") ? obj->query("unit") :"個"),
