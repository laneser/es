...
string query_c_name()
{
  ...
  if (this_player()->query_stat("int") < 10 ) {
    ...
    set("id",({"plant","tree","hwai"}) );
    set_long("根據你仔細鑑定的結果, 你判斷這是一棵槐樹(hwai),\n"
             "如果你有藥鋤, 還可以挖(dig)一點回去。\n");
    set_name("chuan chiung","槐樹");          
    return "槐樹";
  }  
  else {
    set("id",({"plant","tree"}) );
