...
string query_long()
{
	if ( (string) this_object()->query("mask_name") != "mask" )
		return "這是一張" + (string) this_object()->query("c_mask_name") + "的面具，作得栩栩如生。\n";
	else
		return "這是一張面具。\n";
}
...
void wear_me()
{
	...
	tell_room(environment(me),sprintf("%s拿起一張面具,開始在臉上搓揉, 不一會兒功夫, 他的樣子全變了 !\n",me->query("c_name") ),me);
	write(sprintf("你拿起一張%s的面具, 開始化裝..\n",(string) mask->query("c_mask_name") ));
...
int stop_equip(object me)
{
	if ( (int) me->query_skill("trick") < 1 ) {
		tell_object(me,"你的易容術太糟了, 還是不要試的好, 免的弄壞面具\n");
		return 1;
	}
	if ( (int) me->query("spell_points") < query_sp_cost(me) ) {
		tell_object(me,"你的精神力太低了, 無法集中精神化裝。\n");
		return 1;
	}
	return 0;
}
