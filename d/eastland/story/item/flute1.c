...
void create()     
{
	...
	set("long",@LONG
這是「羽民族」之寶－飛翔之笛，它能帶你飛往天都(cast-me)。
LONG
);
 	set_name( "fly flute", "飛翔之笛" );
        add("id",({"flute"}));
	set( "unit", "個" );
	set("weight", 40);
...
int cast_me(string arg)
{
     ...
     if ( !living(player) )
        return notify_fail("請撿起來再使用。\n");
     if ( !query("flute_left") )
        return notify_fail("此飛翔之笛的能源已經用盡了。\n");
     env=environment(player);
     if ( !env->query("can_use_flute") )
        return notify_fail("你無法在這個地方使用飛翔之笛！\n");
     tell_object(player,set_color(
        "\n你手中的「飛翔之笛」發出嗡嗡嗡的鳴聲，閃爍著銀光，並且長出了巨大的雙翼！\n\n","HIW"));
     tell_room(env,set_color(sprintf(
        "\n%s手中的「飛翔之笛」發出嗡嗡嗡的鳴聲，閃爍著銀光，並且長出了巨大的雙翼。\n\n",
        CNAME(player)),"HIW"),player);
     if ( env->query("goto_palace") ) {
       target=SPALACE"plain1";
       msg="一陣刺眼的銀光之後，「飛翔之笛」把你帶到了天都！\n\n";
     }
     else {
        target=SAREA"mountain2";   
        msg="一陣刺眼的銀光之後，「飛翔之笛」把你帶離開了天都。\n\n";
     }
...
string query_short()
{
     return sprintf("飛翔之笛%s", (!query("flute_left") ? " (能源耗盡)" : "") );
}
