...
void create()
{
    set_name("Wand of Ultimate","永恆之杖");
    add("id",({"wand","ultimate wand"}) );
    set_short("永生之杖");
    set_long(@DESC
這把魔杖的杖身是由神密的米蘇理金屬打造而成,它的前端部份鑲著一
顆水晶球,自古相傳這顆水晶球中藏著永恆生命的秘密.矮人的傲人礦物,
半身人的工藝技術,再加上地精的神聖魔力,使這把法杖成為魔法師心中
夢寐以求的武器.
DESC
);
    set("unit","把");
    ...
    set("special_c_msg","前端的水晶球發出一陣強光,包圍住");
    set("no_sale",1);
...
int wand_damage(object victim,int damage)
{
    ...
    if(!(!damaged))
        {  tell_object(holder ,
                "你見到破裂的水晶球閃過一陣異樣的光芒,然後立刻消失.\n") ;
           tell_room( environment(holder), 
                sprintf("\n%s手上%s前端破掉的水晶球發出一陣異樣的光芒,然後立刻消失\n",
		holder->query("c_name"),query("c_name"))
                    ,holder ) ;     
