...
int cmd_stat(string name)
{
	...
	if( tempstr ) write(
		"狀  態: "+to_chinese(tempstr)+to_chinese(ob->query("class"))+
	...
	write("經  驗: "+ob->query_experience() + " (" + exp_reward + ")\n");
	write("探險度: "+ob->query_explore_points()+" 點\n");
	write("陣  營: "+(int)ob->query("alignment") + " (" 
	...
	if( ob->query("max_fp") ) write( 
		"內  力: "+ob->query("force_points")+"/"+ob->query("max_fp")+"\n");
        if( ob->query("max_tp") ) write( 
		"交談能力: "+ob->query("talk_points")+"/"+ob->query("max_tp")+"\n");
