...
void effect(object player)
{
	...
	tell_object(player, 
	    set_color("在沙漠中, 你感到口乾舌燥, 頭昏眼花,.....\n", "HIY", player));
	tell_room( environment(player),
		player->query("c_name") + "因缺水, 似乎快要撐不住了....\n",
		player );
...
void special_effect(object player, int die) 
{
	if( die ) {
        tell_object(player, 
            set_color("你長時間缺乏水分維持你的身體機能, 終於導致死亡!!\n",
            "HIY",player));
...
varargs void remove_effect(object player, int silent)
{
	if( !silent )
		tell_object(player, 
			set_color("離開沙漠, 你終於深深的吸了一口氣, 覺得舒服多了...\n","HIC",player));
...
varargs void apply_effect(object player, int frequency, int damage)
{
	...
	if( !cond_data ) {
		...
		if( frequency < cond_data[0] ) {
			...
			tell_object( player, 
	set_color("你感到嘴唇和喉嚨越來越乾, 彷佛就要裂開了。\n","HIY",player));
		}
