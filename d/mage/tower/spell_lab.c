...
這是銀色之塔中專門研究魔法的研究室, 在這裡你可以將你使用法術的心得做
一番整理。你看到一整排的書桌, 一些初級魔法師正一邊喃喃自語並將自己的心得
寫在自己的魔法書上。你可以在這裡整理你對魔法的心得, 提升(advance) 你的魔
法熟練度。
C_LONG_DESCRIPTION
	);
    set( "no_monster", 1);
...
int do_advance(string arg)
{
	...
	if( exp < need_exp ) {
		...
		write(sprintf("經過一番整理, 你將你對%s的心得仔細地寫在魔法書上。\n",to_chinese(arg)));
		return 1;
	}
}
...
{
	int i;
	write("提升各類魔法熟練等級所需的魔法經驗...\n");
	for( i=0; i<MAX_GUILD_LVL; i++ ) {
		printf("  魔法熟練等級 %2d : %15d 點魔法經驗。\n",i+1,spell_level_exp[i]);
	}
	return 1;
}
