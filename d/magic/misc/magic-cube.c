...
void effect( object caster, int level )
{
    ...
    if( level < 1 ) { 
      tell_object( caster, set_color("你口中唸唸有詞，一個發著奇異光芒的魔法方塊出現在你的身旁。\n","HIW"));

	  tell_room( environment(caster), sprintf("%s口中唸唸有詞，一個發著奇異光芒的魔法方塊出現在他的身旁。\n",c_name) ,caster);
	m_load = modify_damage(caster, 400, TYPE, 20);
	if (m_load < 50) m_load = 50;
	ob->set("max_load",m_load);
	ob->move(caster);
	} else if( level < 2 ) {
	  tell_object( caster, set_color("你口中唸唸有詞，一個發著奇異光芒的大型魔法方塊出現在你的身旁。\n","HIW"));

	  tell_room( environment(caster), sprintf("%s口中唸唸有詞，一個發著奇異光芒的大型魔法方塊出現在他的身旁。\n",c_name) ,caster);
	m_load = modify_damage(caster, 600, TYPE, 30);
	if (m_load < 50) m_load = 50;
	ob->set("max_load",m_load);
	ob->move(caster);
	} else if( level < 3 ) {
      	  tell_object( caster, set_color("你口中唸唸有詞，一個發著奇異光芒的巨大魔法方塊出現在你的身旁。\n","HIW"));
	  tell_room( environment(caster), sprintf("%s口中唸唸有詞，一個發著奇異光芒的巨大魔法方塊出現在他的身旁。\n",c_name) ,caster);
...
int cast(int level)
{
	...
	if( (int)me->query_skill(TYPE) < query_need_skill(level) ) {
	  tell_object( me, sprintf("你的%s技能不足以使用這等級的魔法! \n" ,to_chinese(TYPE)));
      return 0;
    }

    if( present("magic cube",me)) {
      tell_object( me, "你已經製造出一個魔法方塊了，別浪費法力了。\n");
      return 0;
    }
