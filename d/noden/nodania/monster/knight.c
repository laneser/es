...
int order_me(string arg)
{
	...
	if (wealth < query("war_score") ) return 
		notify_fail("你身上的錢不足以支付他的薪水。\n");
	if (!id(arg)) return notify_fail("你想命令誰聽你指揮?\n");
	if (!master->query("knighthood"))
		return notify_fail("你沒有封爵, 憑什麼命令別人?\n");
	if ( master->query("war_score")<query("war_score")*10)
		return notify_fail("你的戰功不夠, 沒資格命令他。\n");
	if ( master->query_temp("now_guard") ) 
		return notify_fail("你已經有一個貼身侍衛了, 還想怎樣?\n");	
	if (!query("body_guard")) {
	...
	write ( sprintf("你拿出%d枚銀幣給你的侍衛當他的出差費:p\n",
		query("war_score") ) );
