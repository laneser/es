...
void drop_eq()
{
	...
	tell_object(this_object(),"突然「轟」的一聲從背後傳來, 你嚇了一大跳。\n");
...
void shift_idle(object me,int time)
{
	...
	if ( time =< (int)target->query_level()/4 ) {
		...
		tell_room(environment(me),
			sprintf("%s蹲在地上抱著頭，喃喃的念著 : 我是%s !! 我是 %s !!\n",
...
int shapeshift(string arg)
{
	object target ;
	if ( ! arg )
		return notify_fail("你想變形成什麼？\n");
	if ( !target = present(arg,envoronment()) )
		return notify_fail("這裡沒有叫 "+arg+" 的東東 !!\n");
//	if ( !living(target) )
//		return notify_fail("
	printf("你對著%s開始聚精會神的冥想 ....\n",target->query("c_name"));
	tell_room(environment(),
		sprintf("%s突然兩眼發直的盯著%s，嘴裡還喃喃的不知道在唸些什麼 ...\n",
