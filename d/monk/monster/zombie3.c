...
void create()
{
	::create();
	set_level(3);
	set_name("8-claw zombie", "八爪僵屍");
	add( "id", ({ "zombie","8-claw" }) );
	set_short("A 8-claw zombie", "八爪僵屍");
	set_long(@C_LONG
八爪僵屍是被超渡失敗的壞人,因怨恨無法消滅而變成的疆屍...
...
C_LONG	);
	...
	set_c_verbs( ({ "%s伸出尖尖的爪子，抓向%s","%s伸長枯骨做的手臂，抓向%s", "%s大聲的笑著說『老禿驢受死吧!』，雙爪劈向%s", }) );
        ...
        set("c_death_msg","八爪僵屍被你殺死後化成一灘血水... \n"); 
...
int block()
{
	...
	tell_object( victim,
"\n\n八爪僵屍突然露出醜陋的奸笑，哇!!! 它身後出現了另外六隻手臂 \n\n");
	tell_room( environment(), 
sprintf("\n八爪僵屍突然背上長出了另外六隻手，一起抓向%s的脖子！\n",victim->query("c_name")),
		({ victim, this_object() }) );
  	for(i=0;i<6;i++)
