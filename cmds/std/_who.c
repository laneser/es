...
*/
                        ...
                        lines += c_dvd;
                        lines += 
                                "目前共有 " + (sizeof(list_wiz)+sizeof(list)) + " 位使用者連線中。";
                        lines += "\t系統負擔: " + query_load_average() + "\n";
            ...
            // linkdead option
                        ...
                        if( !sizeof(list) ) {
                                printf("%s%s%s",c_dvd,"目前沒有斷線的玩家。\n",c_dvd);
                            return 1;
                        }
                        list = sort_array(list, "sort_users", this_object());
                        printf("%s%s%s",c_dvd,"  目前斷線的玩家有: \n",c_dvd);
                        i = sizeof(list);
...
//      list = sort_array(list, "sort_users", this_object());
        ...
        printf("目前共有 %d 位使用者連線中。",(sizeof(list_wiz)+sizeof(list)));
...
列出當前在線的玩家與巫師.
加入-l參數可列出更詳細的資料，加入-d可列出已經斷線的玩家.
HELP
        );
        return 1 ;
}
