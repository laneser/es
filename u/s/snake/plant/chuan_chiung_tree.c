...
void create()
{
   //  以下四項均為外形的描述， realname 才是成功的辨識後傳回的名字     
	...
	set_short("@@query_c_name");    
        ...
        set("c_realname","川芎樹");         // 玩家辨認成功傳回的名字
        set("realname","chuan chiung");
   	...
   	set("value", ({0, "silver"}) );       // 野生的樹當然不值錢啊
        ...
        set("herb_file","chuan_chiung.c") ;   //提煉出來的東東的完整檔名
...
string query_c_name()
{
  ...
  if (this_player()->query_stat("int") < 10 ) {
    ...
    set("id",({"plant","tree","chuan chiung","chuan","chiung"}) );
    set_long("根據你仔細鑑定的結果, 你判斷這是一棵川芎(chuan chiung)樹,\n"
             "如果你有藥鋤, 還可以挖(dig)一點回去。\n");
    set_name("chuan chiung","川芎樹");          
    return "川芎樹";
  }  
  else {
    set("id",({"plant","tree"}) );
