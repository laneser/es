...
void create()
{
    ::create();
      ...
      set_long(
      "定遠縣人，外號神機軍師，使一柄三尖兩刃刀，武藝高強且精通陣法．\n"
      "為梁山泊一同參贊軍務頭領。\n"
      );
...
command("say "+"你已經通過甄試了，不要來耍寶好嗎?");
return 1;
} 
command("look "+(string)this_player()->query("name"));
command("say "+"如果你想加入山賊，首先你要先證明你的能力。你準備好了嗎?");
...
//    delete_embattle_effect(this_object());
    ...
    tell_room(environment(this_object()),
        C_NAME(this_object())+"笑道:「恭喜你，你通過了資格的甄試，我開個證明給你吧」\n"
        "說完，朱武從懷中摸出一張紙片，交給"+killer->query("c_name")+"\n\n");
    tell_room(environment(this_object()),
        C_NAME(this_object())+"無奈地嘆道:「你把我打成這樣，我要去休息休息了....」\n");
    }
    else {
    tell_room(environment(this_object()),
    C_NAME(this_object())+"喝道:「快結陣」!\n\n\n");
    tell_room(environment(this_object()),
    C_NAME(this_object())+"嘆道:「怎麼都跑光了? 三十六計，還是走為上策....」\n");
    }
    command("wave");
    command("drop all");
    ::die(1);
