...
void view_catalog()
{
	...
	printf( "%-31s  %-12s %-7s\n",
		chinese_mode? "武學名稱": "Gonfu", chinese_mode? "武術分類": "level",
		chinese_mode? "所需技能值":"skill levels" );
	write( "=====================================================================\n");
	gonfus = query_gonfus();
	s = keys(gonfus);
	for( i=0; i<sizeof(s); i++ ) {
		printf( "%-30s   %5s     %6d\n",
			...
			chinese_mode? to_chinese(gonfus[s[i]][0]): gonfus[s[i]][0],
			gonfus[s[i]][1]);
	}
	write( "=====================================================================\n");
}
...
int do_train(string arg)
{
	...
	if( tmp > 0 ) {
		write( chinese_mode?
			"你現在要學此武術，還需要 "+tmp+" 點經驗。\n":
			"You need "+tmp+" experience to train the gonfus.\n" );
		return 1;
	}

	write( chinese_mode?
		" *** 經過一番苦練，你終於學到了 "+ to_chinese(arg) + " *** \n":
