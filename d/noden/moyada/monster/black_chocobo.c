...
void create ()
{
	...
	seteuid( getuid() );
   set_level(18);
	set_name( "black chocobo", "黑色陸行鳥" );
	add ("id", ({ "bird", "chocobo", }) );
	set_short( "黑色陸行鳥" );
	set_long( @C_LONG
變種的陸行鳥，不但可以在地上跑，還能在空中飛。可惜飛行速度並不是
很快，時速大約只有三十公里。看它拼命振翅的樣子，滑稽極了。
傳說它酷食任何東西。又如果你是它的主人, 它會幫你撞擊(hit)敵人。
C_LONG
	);
   set( "alignment", 1500 );
    set( "unit", "只" );
   ...
   set_natural_armor( 100, 50 );
	setenv( "C_MIN", "突然一隻$N快速地飛了進來。");
	setenv( "C_MOUT", "$N振翅往$D邊飛走了。");
	set ("special_defense",
	    ([ "all" : 25 ]) );
	set ("wimpy", 100);
	set_skill( "dodge" ,100);
   ...
   set( "mountable", 0 );
   set( "max_load", 2400 );
   set( "c_extra_look", "$N正騎著一隻黑色陸行鳥。\n");
    set ("weight", 900);
   set( "exp_reward", 5555 );
    set ("time_to_heal", 5);
    set_c_verbs( ({ "%s撞到%s身上", "%s一腳踢在%s臉上", "%s一爪抓到%s",
                    "%s用大嘴啄%s", }) );
    set_c_limbs( ({ "身體", "翅膀", "嘴", "腳", "屁股", "爪子", }) );
    set( "c_death_msg", "%s說: 我咧#@!&$*? :( .... 然後就死了。\n" );
}
...
int catch_huntee( object who )
{
    tell_room( environment(this_object()),
      "黑色陸行鳥尖叫一聲: 又是 "+who->query("c_name")+" 這殺人魔，快逃!\n"
      ,  ({ this_object(), who }) );
    tell_object( who, "黑色陸行鳥尖叫一聲: 又是你這殺人魔，快逃!\n" );
    return 1;
}
...
int hit_enemy(string arg)
{
   ...
   if( !living(owner) ) 
        return notify_fail("黑色陸行鳥不聽你的指揮。\n");
   ...
   if( (int)target->query("hit_points")<50 )
        return notify_fail("黑色陸行鳥不屑欺負如此不堪一擊的傢伙。\n");

   if( target->query_temp("dodge_hit") ) {
        tell_object( owner,
            "\n你命令黑色陸行鳥向對方撞去, 但是敵人閃過了!\n\n" );
        tell_object( target,
            "\n黑色陸行鳥向你撞來, 但是你閃過了!\n\n" );
        tell_room( owner,
            owner->query("c_name")+"騎著黑色陸行鳥撞向"+target->query("c_name")+"\n\n",
        ({ owner,target }) );
   ...
   dam = rid/2 - deb/5 -random(dod)/10;
        tell_object( owner,
            "\n你命令黑色陸行鳥向對方撞去, 敵人似乎受到巨大的傷害!\n\n" );
        tell_object( target,
            "\n黑色陸行鳥向你撞來, 你似乎受到巨大的傷害!\n\n" );
        tell_room( owner,
            owner->query("c_name")+"騎著黑色陸行鳥撞向"+target->query("c_name")+"\n\n",
        ({ owner,target }) );
