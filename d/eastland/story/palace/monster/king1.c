...
void create ()
{
	::create();
        set_level(19);
        set_name( "The King Huang", "黃帝" );
        add ("id", ({"king","huang","Huang"}) );
        set_short( "中央天帝 黃帝 [騎在一隻[1;32m□[1;36m□[0;37m[1;32m□[1;36ms[m上]");
	set_long(@CLONG
黃帝生得身材修長，雙手過膝，粉面朱唇，細眉鳳目，兩耳垂珠，鼻似瓊瑤，一身
穿著極為乾淨，遠望極具威儀，細看是一彬彬文生，分別給人不同的感覺。黃帝是
天界眾神的管理者，具有無上權威，他在餘暇常常喜歡在後花園裡遊玩。  　　
CLONG
	);
...
int stop_attack()
{
   ...
   max=sizeof(atk);
   for ( i=0;i<max;i++ ) {
        tell_room( env,sprintf("%s衝到黃帝面前，開始保護他!!\n"
          ,atk[i]->query("c_name")) );
...
void king()
{
   ...
   tell_object(inv,"\n黃帝大吼一聲震得你魂飛魄散！！\n\n\n");
   tell_object(temp1,"忽然從涼亭處傳來一陣嘶吼！\n");
   king_hurt_all(inv);
   king_hurt_all(temp1);
}
int my_tactic()
