...
do_cut(string arg)
{
	...
	if (sscanf((string)corpse->query("short"),"%s的屍體",tmp) < 1 || tmp == "腐爛") {
		return notify_fail("這屍體已經發臭了, 做成面具可能會得皮膚病, 你敢嗎?? \n");
	}
	...
	if ( check_skill(this_player()) ) {
		tell_room(env,sprintf("%s在屍體上劃幾刀, 取下某樣東西。\n",this_player()->query("c_name") ), this_player() );
		write(sprintf("你小心翼翼的從屍體上取下臉皮, 順利的完成了%s的面具。\n",(string)mob->query("c_name") ));
		mask = new(MASK_OBJ);	
	...
	tell_room(env,sprintf("%s試著從屍體上取下臉皮, 結果失敗了!\n",this_player()->query("c_name") ), this_player() );
		corpse->remove();
		mob->remove();
