...
void create()
{
	...
	set_short( "國王耶拉曼十一世" );
	set_long(@LONG
你見到了國王耶拉曼十一世本人，他大約四、五十歲，看起來威嚴中帶點
和藹，當你目光和他交接時耶拉曼對你微微一笑。如果你累積了足夠的戰
功可以向國王要求(request)敕封爵位。
LONG
	);
	set( "race", "human" );
	set( "gender", "male" );
...
void defend_king(object obj)
{
	...
	//	&& (!atk || member_array( inv[i], atk )==-1) ) {
			tell_room( env,
				inv[i]->query("c_name") + "衝到國王面前，準備為保衛國王而戰！\n",
				inv[i] );
...
int request()
{
	...
	if( nws <= order ) {
		tell_room( environment(),
			"\n國王說道: 你的戰功只能享有你目前的身份地位，繼續努力吧。\n");
		return 1;
	}
	
	tell_room( environment(),
		"\n國王高舉他的佩劍，在" + this_player()->query("c_name") +
		"的頭上及肩膀各劃了一個圓圈。\n"
		"\n接著，他用宏亮的聲音宣佈:\n"
		"\n    現在，我封你為" + this_player()->query("c_name") + 
