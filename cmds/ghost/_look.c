...
// do we have an environment?  If not, write the default message
		if (!env) {
			write (chinese_mode?"這裡一片虛無縹渺，看不到任何東西。\n":"There is nothing to see here.\n");
			return 1;
		}
/*
...
string living_description (object ob)
{
	...
	if (stringp (race) && race != "")
		reg += chinese_mode? genat + "的種族是" + raceat + "。\n":
			genat + " " + raceat + ".\n";
...
/* okay, this is the way to get things built into the room.
   ...
   dummy items.  Fix to item_desc of @@fun by Grendel.*/
	...
	if (!ret || ret == "")
		return chinese_mode? "你想看什麼？\n": "I don't see that here.\n";

	if (it) {
		if (it != viewingOb) {
			if (living (it))
				tell_object( it,
					user->query("c_name") + "正盯著你看，不知道有什麼企圖....。\n");
			foo = it->query("name");
...
//Check if the user is blind.
	if ((int)user->query ("blind"))      
		return chinese_mode?"你已經瞎了，什麼也看不見。\n":"You are blind and cannot see.\n";
...
//Guess there is no light here, then.
	return chinese_mode?"這裡一片漆黑，伸手不見五指。\n"
...
*/
		...
		case 0:
			...
			break;
		case 1:
			long += (chinese_mode?"\t這裡唯一的出口是: "+dirs[0]+"。\n"
								 :"\tThe only obvious exit is "+dirs[0]+".\n");
			break;
		case 2:
			long += (chinese_mode?"\t從這裡你可以往 "+dirs[0]+" 或往 "+dirs[1]+"。\n"
		...
		default:
      		if(!chinese_mode)
				...
				long += "\t從這裡你可以往 " + implode(dirs[0..(i-2)], "、") + " 或往 " + dirs[i-1] + "。\n";
		}
	}
...
string process_contents (mixed *contents, int flag)
{
	...
	while (loop--) {
		...
		if (i > 1) {
			...
			tmp = chinese_mode?
				sprintf ("%s%s%s (%s)", convert_c_number(i), (unit=obj->query("unit"))?unit:"個",
...
string convert_c_number(int num) {
  if (!c_number)
    c_number = ({
"一", "兩", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二",
"十三", "十四", "十五", "十六", "十七", "十八", "十九", "二十" });
   
  return ((num > 20) ? "許多" : c_number[num - 1]);
}
