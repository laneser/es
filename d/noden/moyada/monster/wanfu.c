...
void create ()
{
	...
	set_short( "店長王虎" );
	set_long(@LONG
王虎是這家商店的老闆，他看起來十分強壯，全身肌肉一塊一塊地，相當
嚇人。他留著一條長長的辮子，裸著上身「親切」的招呼著客人。噢！他還隨
身攜帶著一把大刀，刀上刻著一條氣宇軒昂的龍。
LONG
	);
    ...
    set( "inquiry", ([
         "job" : "@@ask_job",
         "devil" : "聽說惡魔常常偽裝成平常人, 需要特殊的物品才能讓他現形。\n",
    ]) );
    
	set ("chat_chance", 5);
	set ("chat_output", ({
	    "王虎用毛巾擦完桌子後便開心地擦拭他的大刀。\n",
	    }) ); 
	set ("att_chat_output", ({
	    "王虎說：小子，你膽敢向我挑戰，有膽！\n",
	    "王虎說：你還不和解嗎? 你不可能打敗我的。\n",
...
int catch_huntee( object who )
{
    tell_room( environment(this_object()), 
      "王虎很不爽的向"+who->query("c_name")+"說: 你還敢進來，活的不耐煩了，我就成全你，死吧!\n"
    ,  ({ this_object(), who }) );
    tell_object( who,
      "王虎很不爽的向你說: 你還敢進來，活的不耐煩了，我就成全你，死吧!\n"
    );
    return 1;
}
...
void ask_job( object who )
{
      if( done )
        ...
        tell_object( who, 
          "王虎說：我的儲物室據說躲著一隻惡魔，客人都不敢上門，你能幫我除掉它嗎?\n");
}
...
int hunt_check( object wanfu, object victim )
{
    ...
    if( victim && !member_group( victim, attackers ) &&
      ...
      tell_room( environment(wanfu), 
        "王虎大笑著說: "+victim->query("c_name")+"，你以為 quit 就能逃出我的手掌心嗎? 死吧!\n"
        , wanfu );
      kill_ob( victim );
      return 1;
    }
...
THANKS
		...
		tell_room( environment(), 
			"王虎說：歡迎光臨，需要什麼嗎？\n" );
	} else if( sscanf( msg, "%s往%s邊離開。", who,str )==2 )
		tell_room( environment(), 
			"王虎說：謝謝光臨，下次再來。\n" );
}
