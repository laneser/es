...
// ([ <spell-name> : ({ <spell_system>, <level_required>, <max_level>, 
//                      <level_per_level>, <exp_proportion(%)>,  }), 
//    <spell-name> : ({ <spell_system>, <level_required>, <max_level>, 
//                      <level_per_level>, <exp_proportion(%)>,  }), 
//                         :
//                         :
...
varargs int can_learn_spell(object player, string spell, int level)
{
	...
	if( !spells || !mapp(spells) || !spell || undefinedp(spells[spell]) )
		return 0;
	if( !level ) {
		if( (int)player->query_level() >= spells[spell][1] )
			return 1;
		else return 0;
	} else {
		if( level > spells[spell][2] ) return 0;
		if( (int)player->query_level() >= spells[spell][1] + spells[spell][3] )
			return 1;
		else return 0;
	}
}
...
int raise_cost(object player, string spell)
{
	...
	level_need = spells[spell][1] + spells[spell][3] * (level+1);
	if( level_need > 19 ) level_need = 19;
	if( (int)player->query_level() < level_need )
		return 0;
	exp = spells[spell][4] * level_need * level_need
	      * (level+1) * (level+1) / 5 ;
...
void view_catalog()
{
	...
	printf( "%-30s  %-15s  %-8s  %-8s\n",
		chinese_mode? "法術名稱": "Spell", chinese_mode? "法術種類": "Type",
		chinese_mode? "所需等級": "Level need", chinese_mode? "最大級數": "Max Lvl" );
	write( "=====================================================================\n");
	spells = query_spells();
	s = keys(spells);
...
int do_study(string arg)
{
	...
	if( !can_learn_spell( this_player(), arg ) )
		return notify_fail( chinese_mode?
			"對不起，你現在的等級還沒有辦法學習這種法術....。\n":
			"Sorry, you cannot learn such spell by now.\n" );
	write( chinese_mode?
		"經過一番耐心的研讀，你學會了初級的" + to_chinese(arg) + "法術！\n":
...
int do_raise(string arg)
{
	...
	if( !arg || arg=="" ) {
		...
		s = keys(my_spells);
		printf( "%-30s  %-8s  %s\n", chinese_mode? "法術名稱": "Spell",
			chinese_mode? "目前等級": "Level", chinese_mode? "提升等級所需經驗": "Exp. Required" );
		write( "============================================================\n");
		for( i=0; i<sizeof(s); i++ ) {
			if( undefinedp( spells[s[i]] ) ) continue;
			exp = raise_cost( this_player(), s[i] );
			if( exp < 1 )
				printf( "%-30s  %6d    目前已達最大等級\n",
					chinese_mode? to_chinese(s[i]) + " (" + s[i] + ")": capitalize(s[i]),
					my_spells[s[i]] );
			else
				printf( "%-30s  %6d    %10d\n",
