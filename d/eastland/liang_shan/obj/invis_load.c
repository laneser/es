...
{
	seteuid(getuid());
	set_name("invis_obj", "隱形物件");
	set_short("invis_obj" ,"隱形物件");
        set_long(
                 "quest 需要的東西( 請看skeleton.c )....\n");
...
{	
     ...
     if( !arg || lower_case(arg) != "skeleton" ) return notify_fail("把什麼放下?\n");
     me=this_player();
     usr = (mixed *)me->query_temp("party_members");
     num = sizeof(usr);
    
     if ( (string)me->query_temp("leader") != (string)me->query("name") ) {
	       write("只有隊伍的領導者，才能下命令喔。\n");
	       return 1;
	       }
	       for( i=0; i<sizeof(usr); i++ )
	       {       
	       str = ((usr[i]->query_temp("carry_skeleton",1))?("一，二，三....你如釋重負地把這具巨大髑髏放下...\n"):"");
	       usr[i]->delete_temp("carry_skeleton");
	       tell_object(usr[i],str );
	       }
	tell_room(environment(me),"你看到"+me->query("c_name")+"和他的隊友\n"
		"將一件龐然大物丟到地上\n", usr);
