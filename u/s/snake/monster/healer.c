...
void create()
{
	...
	set_short( "刑老爹" );
	set_long(
		"刑老爹本來的名字已經沒有人知道了，只知道他原來是東方大國\n"
		"殺手組織中的一員，因為背叛組織而遭到刺瞎雙眼的處罰，刑老\n"
		"爹為了避禍，來到這裡隱姓埋名，希望能安度晚年。\n"
	);
	set_temp( "aided",0);
	set( "race", "human" );
	set( "gender", "male" );
...
void relay_message(string class, string msg)
{
   ...
   me=this_object();
   if( sscanf( msg, "%s往%s離開。",who,str)==2)
   {
...
//   }   
   if ( (bleed = me->query("conditions/bleeding")) && 
      ...
      tell_room(environment(me), "刑老爹替刑老爹的傷口止血。\n");
   ...
   if ( ( hp < 300 ) && ( random(10) < 3 ) ) {
      if (query_temp("aided")==0) {
        ...
        tell_room(environment(me),"刑老爹對自己進行急救....\n");
...
void astrquest(object ob)
{
   ...
   write(
         "邢老爹想了一下子後說道:我好像曾給過這麼一個東西給歐瑞安長老,\n"
         "    但是我忘了在那找到那個東西的,只記得那個東西叫“優波羅花”。\n"
         "    至於在那會有,我就不清楚了。\n"
         "邢老爹話一說完就不理你，專心回去做他的事了。\n"
         );
