...
int put_into(string str)
{
	...
	if(sscanf(str,"%d %s into %s",num, athis, that)==3) {
		...
		if( !this_player()->debit(athis, num) ) {
			write( "你沒有那麼多" + to_chinese(athis+" coin") + "。\n") ;
			return 1;
		}
		...
		write( sprintf( "你把 %d 枚%s放進%s。\n", num, to_chinese(athis+" coin"), query("c_name")) );
		tell_room( environment(this_player()),
			this_player()->query("c_name")+"把 "+num+" 枚"+to_chinese(athis+" coin")+"放進"
				+ tht->query("cap_name")+"。\n",
			this_player());
		return 1 ;
	}
...
// If the player is carrying the container, his capacity has to go down.
		...
		write( "你把" + ths->query("short") + "放進" +tht->query("short")+ "裡。\n");
		tell_room( environment(this_player()),
			this_player()->query("c_name") + "把" + ths->query("short") + "放進" + tht->query("short") + "。\n",
			this_player());
		return 1;
	}
...
int get_from(string str)
{
	...
	if (!str) return notify_fail( "指令格式: get <東西> from <容器>\n");
...
// add by Ruby@ES at 95/11/24
	...
	if ( sscanf(str, "%d %s from %s", num, this1, that )==3 ) {
		tht = present( that, this_player());
		if(!tht) tht = present(that, environment(this_player()));
		if(!tht) return notify_fail( "這裡沒有任何叫做 " + that + " 的東西。\n");
		if( tht!=this_object() ) return 0;

		// Is it closed?
		if( !tht->receive_objects() ) return notify_fail( "你必須先將它打開。\n");

		if( tht->query("prevent_get") ) return notify_fail( "你不能從那裡面拿走任何東西。\n");

		ths = present(this1, tht);
		if (!ths) return notify_fail( "那裡面沒有這樣東西。\n");

		if (ths->query_number()<num )
			return notify_fail( "那裡面沒有這麼多"+to_chinese(this1, " coin") +"。\n");

		this_player()->credit( ths->query_type(), num );
		ths->set_number( (int)ths->query_number()-num );

		if (num==1) word="coin"; else word = "coins";
		write( "你從"+tht->query("c_name")+"中拿出 "+num+" 枚"+to_chinese(this1+" coin")+"。\n" ) ;
		tell_room( environment(this_player()),
			this_player()->query("c_name")+"從"+tht->query("c_name")+"中拿出 "+num+" 枚"+to_chinese(this1+" coin")+"。\n",
			this_player() );
		return 1;
	}
			...
			if (tht->receive_objects()) {
				if (this1 != "all") {
					...
					if((int)ths->query("prevent_get"))
						return notify_fail( "你不能從那裡面拿走任何東西。\n");
...
// down, but the bulk does not go down.
						...
						write ( "你從"+tht->query("short")+"中拿出"+c_word+"。\n");
						tell_room( environment(this_player()),
							this_player()->query("c_name")+"從"+
							tht->query("short") + "中拿出"+c_word+"。\n",
							this_player() );
						return 1;
					}
					if( res == MOVE_NOT_ALLOWED )
						notify_fail( "你沒有辦法拿起這樣東西。\n");
					if( res == MOVE_NO_ROOM )
