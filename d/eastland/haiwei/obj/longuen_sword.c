...
void create()
{
	set_name( "Sword of Longuen", "龍淵之劍" );
	add( "id", ({ "longuen", "sword" }) );
	set_short( "Sword of Longuen", "龍淵之劍" );
	set_long(
		"You see a longsword with freezing blue blade.\n",
		"這是一把有著異樣深藍色劍刃漾著寒光的古劍，劍柄上刻著四個篆字「龍淵神劍」。\n" );
	set( "unit", "把" );
	set( "type", "longblade" );
...
int weapon_hit( object victim, int damage )
{
	...
	if( random(intelligence)>=15 && sp > 20 ) {
		tell_object( owner, can_read_chinese(owner)?
			"\n你的龍淵之劍突然發出一道青紫色的弧光，劍身變成明鏡一般的銀色！\n\n":
		...
		tell_room( environment(owner), ({
			...
			"\n" + owner->query("c_cap_name") + "的龍淵之劍突然發出一道青紫色的弧光，劍身變成明鏡一般的銀色！\n\n"}),
			owner );
