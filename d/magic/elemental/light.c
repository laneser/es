...
int cast(int level)
{
	...
	if( (int)me->query_skill("elemental") < query_need_skill(level) ) {
	  tell_object( me,"你的元素魔法技能不足以使用這等級的魔法! \n" );
      return 0;
    }
    inv=all_inventory(me);
    inv=filter_array(inv, "filter_inv", this_object());
    if( sizeof(inv) ) {
      tell_object( me, "你已經製造出一顆光球了，別浪費法力了。\n");
      return 0;
    }
     else
     {
	write("你施展照明術，一團發光粒子逐漸在你的手中形成一顆光球。\n");

	tell_room( environment(me), sprintf("%s喃喃地念了一段咒文，一些發光的粒子逐漸在%s的手中形成一顆光球。\n",me->query("c_name") ,me->query("c_name") ),me);
