	...
	if( weapon ) {
		...
		if( ft= (string)me->query("force_type") )
			victim->receive_special_damage( ft, damage );
                        ...
                        victim->receive_damage( damage );
		switch ( damage / 10 ) {
			case 0 : c_msg = "輕微的";     break;
			case 1 : c_msg = "一點";       break;
			case 2 : c_msg = "不小的";     break;
			case 3 : c_msg = "頗為嚴重的"; break;
			case 4 : c_msg = "嚴重的";     break;
			case 5 : c_msg = "相當嚴重的"; break;
			case 6 : c_msg = "十分嚴重的"; break;
			default: c_msg = "致命的";     break;
		}
		if( damage ) {
		  tell_object( victim,
                         sprintf("你被%s的內力震傷, 受了%s內傷!\n",me->query("c_name"),c_msg)
		  );
		  tell_object( me, 
                        sprintf("你用內力將%s震傷, 對方受了%s內傷!\n",victim->query("c_name"),c_msg)
		  );
		}
	}
}
