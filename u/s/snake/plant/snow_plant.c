...
void create()
{
	set_short("@@query_c_name");    
   	...
   	set("value", ({0, "silver"}) );       // 野生的樹當然不值錢啊
        ...
        set("herb_file","snow_lotus.c") ;     //提煉出來的東東的完整檔名
...
string query_c_name()
{
  ...
  if (this_player()->query_stat("int") < 10 ) {
    ...
    set_long("根據你仔細鑑定的結果, 你判斷這是極少見的雪蓮(snow lotus),\n"
             "如果你有藥鋤, 還可以挖(dig)一點回去。\n");
    return "雪蓮";
  }  
  else {
    set("id",({"plant","tree"}) );
    set_name("a plant","多年生草本植物");
    set_long("根據你在國中生物課本中得來的知識, 你判斷這是一種多年生草本植物, \n"
             "不過正確的名稱, 還須要進一步的辨識。\n");
    return "多年生草本植物";
  }
}      
