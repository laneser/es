// default function for spells. Create by Kyoko. Modified by Yueh
#include <mudlib.h>
#include <magic.h>
...
int success_hit(int number, object caster, string type, int need_spell_level)
{
   ...
   if( random(number) > skill ) return 0;
   if (caster->query_temp("cast_flee"))
       {
          caster->delete_temp("cast_flee");
	n=caster->query_temp("cast_miss");
	if (!n) n=0;
          if ( random (100) > (spell_lvl+40+n*2+((spell_lvl - need_spell_level) / 5))) {
		caster->set_temp("cast_miss",n+1);
		return 0;
	}
        }
   caster->set_temp("cast_miss",0);
   return 1;
}
...
void gain_spell_experience(object caster, string type, int exp)
{
	...
	if (environment(caster)->query("PK_ZONE")) return; 
...
int check_level(object caster, object victim, int one)
{
	...
	if( one ) {
		...
		tell_object( victim, set_color(
			sprintf("%s(%s)嘗試攻擊你, 但是萬能的天神阻止了他並給予警告。\n",
				caster->query("c_name"), caster->query("name")), "HIC", victim));
		tell_room( environment( victim ), 
			sprintf("%s(%s)嘗試攻擊%s(%s), 但是萬能的天神阻止了他並給予警告。\n",
				caster->query("c_name"), caster->query("name"),
				victim->query("c_name"), victim->query("name")) ,
				({ caster, victim }));
	} else
		tell_object( victim, set_color(
			"天上突然傳來一陣宏亮的聲音: 你的等級太低了，不能被 PK。\n"
			"萬能的天神偉大的魔力讓你免於受到傷害。\n", "HIC", victim));
	return 0;
}
