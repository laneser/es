...
int cast(int level)
{
    ...
    if( me->query_temp("effect/heart-protect-shield") ) {
      tell_object( me, "你上一個魔法護心盾還沒消失呢! 別浪費法力了。\n" );
      return 0;
    }
	ali = (int) me->query("alignment") ;
	if ( ali > -200){
		write( "你施展護心之盾咒文，但是卻沒有任何事發生。\n");
		tell_room( environment(me), me->query("c_name") + "喃喃地念了一段咒文，但是卻沒有任何事發生。\n" , me);
	return 1;
	}

	write( "你施展護心之盾咒文，在你的面前逐漸形成一層無形的力場。\n");
...
{
    if( !caster ) return;
    tell_object( caster , "你感覺到你的魔法護心盾似乎消失了。\n" );
