...
int cmd_hp()
{
	...
	msg = "";
		if( max = me->query("max_hp") ) {
			...
			msg = sprintf(sprintf(" 體  力  :   %s/%s",set_color("%4d", query_color(flag,max))
				,set_color("%4d\n", "HIC")), flag, max);
		}
		...
		if( max = me->query("max_fp") ) {
			...
			msg = sprintf(sprintf("%s 內  力  :   %s/%s",msg,set_color("%4d", query_color(flag,max))
				,set_color("%4d\n", "HIC")),flag, max);
		}
		if( max = me->query("max_tp") ) {
			flag = me->query("talk_points");
			msg = sprintf(sprintf("%s交談能力 :   %s/%s",msg,set_color("%4d", query_color(flag,max))
				,set_color("%4d\n", "HIC")), flag, max);
		}
/*		if( max = me->query("max_ap") ) {
			flag = me->query("action_points");
			msg = sprintf(sprintf("%s行動能力 :   %s/%s",msg,set_color("%4d", query_color(flag,max))
				,set_color("%4d\n", "HIC")), flag, max);
		}
*/
