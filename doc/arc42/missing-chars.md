...
全樹 `*.c` 原有 945 處 `□`，已依上下文補回 690 處（見 git log）。
這份是**剩下的 255 處**，給熟悉遊戲設定的人逐一認字用。
重新產生統計：`grep -rn '□' --include='*.c' .`

| 類別 | 處數 | 檔數 | 怎麼辦 |
|---|---|---|---|
| C 猜不出原字 | 172 | 86 | 靠上下文認，認出來直接改 |
...
**`d/eastland/saulin_temple/obj/cloth3.c`**

- `:7` set_name( "wen-su robe", "文殊□衣" );
- `:9` set_short( "文殊□衣" );
- `:11` _(2 個)_ "□衣是祭祀神佛時所穿的衣服。文殊□衣是祭拜文殊菩薩時穿的長袍。\n"
