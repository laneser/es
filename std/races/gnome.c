...
// This is a race body for Gnome, Created by Annihilator,
// Updated by Kyoko.
...
void setup_race_body()
{
	...
	// Gnome has no any special defense.
	body->set("special_defense", ([ "cold": -8, 
		"poison": 23, "mental": -10,
		"magic": 10, "evil": -20, "divine": 5, ]) );

	// effect the critical medication resistance.
	if( userp(body) ) body->set("constitution", 25);

	// the natural age of Gnome.
	if( userp(body) ) body->set("natural_age", 50);
...
// These functions are used for race Gnome.
// Please don't cover the functions in user.c.

int query_natural_life() { return 320; }

// define the cost of advance stats for Gnome.
int query_stat_exp_cost(string stat, int val)
{
	mapping stat_exp_rate = ([
		"str": 120, "int": 120, "dex": 100,
