...
int cmd_wield(string str, int silent)
{
	...
	if( obj==firstwep || obj==secondwep )
		return notify_fail ("你已經裝備這個武器了。\n");

	if( firstwep && secondwep )
		return notify_fail ("你最多只能同時裝備兩種武器。\n");

	if( firstwep && shield )
		return notify_fail("要裝備第二種武器，你必須空出一隻手。\n"
		);
	
	if( !firstwep && !secondwep ) {
		if( shield && (int)obj->query("nosecond")==1 )
			return notify_fail("這個武器必須用雙手才能使用。\n"
			);

		if( !silent ) {
			write( sprintf("你裝備%s為武器。\n",obj->query("short")));
	...
	if( firstwep->query("nosecond") )
		return notify_fail("你已經裝備著需要雙手的武器了。\n"
		);

	if( obj->query("nosecond") )
		return notify_fail(sprintf("%s需要用雙手使用。\n",obj->query("short"))
		) ;

	if( !obj->query("second") && !firstwep->query("second") )
		return notify_fail("你不能同時使用兩種主要武器。\n"
		);

	if( (int)obj->query("second") ) {
		usr->set("weapon2", obj);
		if( !silent )
			write(sprintf("你裝備%s為次要武器。\n",obj->query("short")));
	} else {
		usr->set("weapon2", firstwep);
		usr->set("weapon1", obj);
		if( !silent )
			write(sprintf("你把%s換下來作次要武器。\n",firstwep->query("short")));
			write(sprintf("改用%s當主要武器。\n",obj->query("short")));
...
這個指令讓你裝被某樣東西做為武器. 你最多可以裝備兩樣武器, 但某些
武器因為重量或使用方法的關係可能需要用到兩隻手. 另外, 你也不能一
手拿盾一手拿雙手武器.

相關指令: wear, remove, unwield.
HELP
	);
	return 1 ;
}
