...
void create()
{
	::create();
	set_level(16);
	set_name( "Sawyer","鋸木工人" );
	add("id",({ "lizardman","sawyer" }) );
	set_short( "鋸木工人" );
	set_long(@LONG
你看到的是一位體格壯碩的蜥蜴人，它是蜥蜴人老工
匠唯一的徒弟，它正準備去找些木材來作傢俱。
LONG
		);
	set( "gender","male" );
	set( "race","lizardman" );
...
void ask_mahogany()
{
	...
	write(@ALONG
鋸木工人說:
喔～你想找桃木啊？只要去桃花林就有啦。不過桃花林很
大，我每次去那奇怪的桃花林都會迷路，所以你最好帶張
地圖和工具到處逛一逛，否則找不到好桃木的。
ALONG
		);
...
void ask_tool()
{
	...
	if( this_player()->query_temp("lizard_quest/tool") ){
  	write(@BLONG
鋸木工人很驚訝的說:	
天啊!!你沒帶工具還想去砍桃木？可是我也要用這把鋸子
。這樣好了，你去找我師父向他借一把鋸子(saw) 來應應
急吧。
BLONG
		);
