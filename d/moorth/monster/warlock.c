...
void create()
{
	...
	::create();
	set_level(19);
	set_name( "Moorth Warlock", "默爾斯戰巫" );
	add( "id", ({ "warlock" }) );
	set_short( "默爾斯戰巫" );
	set_long(
		"你看到一個默爾斯的「戰巫」，他們是屬於拉曼西斯女皇的親衛隊，不但\n"
		"施法能力不下於魔法師，高超的劍術更是令人不寒而慄！\。\n"
	);
	set( "unit", "名" );
	set( "race", "human" );
	set( "gender", "male" );
...
// set( "war_value", 250 );
   ...
   set_skill( "two-weapon", 100 );
	...
	set( "chat_chance", 20 );
	set( "att_chat_output", ({
		"默爾斯戰巫大叫: 你們這些愚蠢無知的諾頓人，通通給我下地獄去吧！\n",
		"默爾斯戰巫說道: 殺死你們這些垃圾簡直就是浪費我的時間！\n",
		"默爾斯戰巫說道: 諾頓王在哪裡？叫他把賽那雷斯之冠交出來！\n"
	}) );
...
int my_tactic()
{
	...
	if( random(20)<4 ) {
	  for( i=0; i<sizeof(victim); i++ ) {
		message( "tell_object",
			"默爾斯戰巫口中念道: Del-Ka'ta Mu-ju'ra .....\n"
			"突然，一道刺骨的寒風夾著無數冰錐吹了過來，你聞到一股血腥的氣味！\n",
			victim[i],
			this_object() );
	  }
