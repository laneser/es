...
int change_perm_stat(object player, string arg, int stat)
{
    ...
    if( !valid_statsd_check(previous_object()) ) return 0;
    if( !living(player) ) return notify_fail("只有生物才有屬性!!\n");
...
{
    if( align < -5000 ) return set_color("惡貫滿盈", "HIR");
    else if( align < -2500 ) return set_color("罪大惡極", "HIR");
    else if( align < -1500 ) return set_color("十惡不赦", "HIM");
    else if( align < -1000 ) return set_color("惡名昭彰", "HIM");
    else if( align < -700 ) return set_color("作惡多端", "HIY");
    else if( align < -400 ) return set_color("惡行惡狀", "HIY");
    else if( align < -200 ) return set_color("素行不良", "HIY");
    else if( align < 200 ) return set_color("無功無過", "HIW");
    else if( align < 400 ) return set_color("見義勇為", "HIG");
    else if( align < 700 ) return set_color("義行可嘉", "HIG");
    else if( align < 1000 ) return set_color("行俠仗義", "HIB");
    else if( align < 1500 ) return set_color("義名遠播", "HIB");
    else if( align < 2500 ) return set_color("義薄雲天", "HIB");
    else if( align < 5000 ) return set_color("功德無量", "HIC");
    else return set_color("一代大俠", "HIC");
}
...
string body_status_string(object obj)
{
    ...
    percent = 100 * med / max;
    if( percent < 20 ) return set_color("你覺得身體一切正常，精神飽滿。", "HIC", obj);
    else if( percent < 40 ) return set_color("你覺得頭有點暈，不過沒有什麼大礙。", "HIY", obj);
    else if( percent < 60 ) return set_color("你覺得頭重腳輕、全身乏力，也許是藥吃太多了。", "HIG", obj);
    else if( percent < 75 ) return set_color("你覺得頭痛欲裂、全身直冒冷汗，最好找個地方休息一下。", "HIM", obj);
    else if( percent < 90 ) return set_color("你覺得天旋地轉，連站都站不穩，好想躺下來睡上一覺。", "HIR", obj);
    else return set_color("你覺得全身上下都不對勁，一點力氣也沒有。", "HIC", obj);
}
...
varargs string status_string(object obj)
{
    ...
    if( !max_hp ) return set_color("狀況不明", "HIR", obj);

    percent = 100 * tar_hp / max_hp;
        if (userp(obj)) {
    if( percent < 0 ) return sprintf("%s。", set_color("死了", "HIR", obj));
    else if( percent < 10 ) return sprintf("已經%s了。", set_color("奄奄一息", "HIR", obj));
    else if( percent < 25 ) return sprintf("渾身是血，受傷極重，%s。", set_color("有生命危險", "HIM", obj));
    else if( percent < 40 ) return sprintf("身上到處都是傷口，%s。", set_color("血流不止", "HIY", obj));
    else if( percent < 60 ) return sprintf("傷到不少地方，%s。", set_color("流了許多鮮血", "HIY", obj));
    else if( percent < 80 ) return sprintf("受了幾處傷，但都%s。", set_color("沒有傷到要害", "HIG", obj));
    else if( percent < 100 ) return sprintf("似乎受了點傷，但是%s。", set_color("看起來並不要緊", "HIG", obj));
    else return sprintf("現在正處於%s。", set_color("最佳狀況，沒有受傷", "HIC", obj));
        } else {
    if( percent < 0 ) return "死了。";
    else if( percent < 10 ) return "已經奄奄一息了。";
    else if( percent < 25 ) return "渾身是血，受傷極重，有生命危險。" ;
    else if( percent < 40 ) return "身上到處都是傷口，血流不止。" ;
    else if( percent < 60 ) return "傷到不少地方，流了許多鮮血。" ;
    else if( percent < 80 ) return "受了幾處傷，但都沒有傷到要害。" ;
    else if( percent < 100 ) return "似乎受了點傷，但是看起來並不要緊。";
    else return "現在正處於最佳狀況，沒有受傷。";
        }
}
