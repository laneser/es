...
void create ()
{
        ...
        seteuid( getuid() );
        set_level(17);
        set_name( "Advisor ZhuGe Not Bright", "軍師" );
        add ("id", ({ "advisor" , "mage" , "bright" }) );
        set_short("牧馬關軍師 諸葛不明" );
        set_long( @C_LONG
牧馬關軍師諸葛不明原是東方大國的司天，掌管觀天象以記事的工作，後來因為
不滿朝政被權臣所把持，就自己請調到牧馬關來。據說他有著呼風喚雨的本領。現在
他正帶著幾個士兵在這兒清點裝備。
C_LONG
        );
    set( "alignment", 1000 );
    ...
    set("c_death_msg","%s 奄奄一息地說道：「和平..奮鬥..救東國..！」然後就掛了...\n");
