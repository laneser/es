...
void check_mobs(object player) 
{
	...
	string msg ;
			...
			sprintf("%-16s - %-12s - %|4s - %|4s - %|7s - %s\n",
					"檔名","中文名稱","LV","HP","AC/DB","WC/MaxD~MinD"));
	write_file(output,
"==========================================================================\n");
	for ( i = sizeof(all_files)-1 ; i >=0 ; i -- ) {
	if ( catch(ob = new(path+all_files[i]))) continue ;
...
{
write(
"用法 : check_objects <分析項目> <輸出檔名>\n\n"
"這是一個分析一個子目錄下所有物件目前狀況的工具。\n"
"它會分析你目前所在的子目錄下的所有檔案，並將結果\n"
"輸出到你的根目錄下叫 weapons.rpt (例) 的檔案。\n\n"
"	分析項目 : weapons ,mobs ,armors.\n");
	return 1;
}
