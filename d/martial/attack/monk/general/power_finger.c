...
string *attack_msg = ({
		"%s力貫指尖，一指向%s點去", 
		"%s回身凝神，一記『力貫長虹』緩緩卷向%s",
		"%s側劈一掌，劃出一招『金剛伏虎』，攻向%s",
		"%s意守丹田，右手『飛鴻揚翼』氣勢恢宏，擊向%s",
		"%s踏出右腳，雙指自上而下攻出，一招『箭指連環』快速攻向%s",   
		"%s雙目圓睜，一股強勁指力湧向%s，乃是『金剛怒目』",
		"%s仰天大喝一聲，劈出『巨浪濤天』大股內勁卷向%s",
		"%s氣貫全身，使出終極一式『金剛破魔』一大股內勁奔湧攻向%s",
});
	...
	if( type < 1 || type > 8 ) return 0;
   	...
   	me->add("gonfus_exp/power-finger", 3*type+1+random(2 * type));
        if (random(350)<lv) { 
           ...
           tell_object( me, set_color("你長嘯一聲，金剛指力發動首次進擊! \n","HIR") ); 
        if (random(180)<lv) { 
           me->add("force_points", query_need_fp());
           this_player() -> continue_attack();   
           tell_object( me, set_color("你長嘯一聲，金剛指力發動再次進擊! \n","HIR") );  
                            }  
                            }
...
varargs string query_attack_msg(object me, object victim, int type)
{
	if( type < 1 || type > 8 ) return 0;
        ...
        return attack_msg[type-1]+"惟出手時顯得內力不足"; 
