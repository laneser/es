...
void invite_member(object le, object me)
{
	...
	if( sizeof(invited)>=MAX ) {
		write("你的隊伍最多只能有 " + MAX + " 個人.　\n");
		return;
	}

	if( member_array(me, members)!=-1 ) {
		write(C_CAPNAME(me)+"已經是你隊伍的成員了.　\n");
		return;
	}

	if( member_array(me, invited)!=-1 ) {
		write("你已經邀請過"+ C_CAPNAME(me)+"了.　\n");
		return;
	}

	invited += ({ me });
	le->set_temp( "party_invited", invited );
	write("你邀請"+C_CAPNAME(me)+"加入你的冒險隊伍.　\n");
	tell_object(me, 
		C_CAPNAME(le)+"邀請你參加"+ to_chinese(POSS(le)) +"隊伍.　\n"
	);
	tell_room( environment(le), 
		C_CAPNAME(le)+"邀請"+C_CAPNAME(me)+"加入"+ to_chinese(POSS(le))+"隊伍　\n",
		({ le, me }) );
...
void remove_invitation(mixed arg)
{
	...
	if( arg[2] ) {
	  if ( ! nullp(arg[1]) ) {
		tell_object(arg[1],
			C_CAPNAME(arg[0])+ "邀請你參加冒險隊伍的時限已經到了.　\n"
		);
		tell_object(arg[0],
			"你邀請" + arg[3]+ "參加冒險隊伍的時限已經到了.　\n"
		);
		} else
		tell_object(arg[0],
			"你邀請"+arg[3]+"參加冒險隊伍的時限已經到了.　\n"
...
void add_member(object le, object me)
{
	...
	if( member_array(me, members)!= -1 ) {
		write( 
			"你已經是" + C_CAPNAME(le) + "的冒險隊伍成員之一了.　\n"
		);
		return;
	}
	if( sizeof(members)>=MAX ) {
		write( 
			C_CAPNAME(le)+"的冒險隊伍人數已經滿了.　\n"
		);
		return;
	}
	if( member_array(me, invited) == -1 ) {
		write(
			"你並沒有被邀請參加"+C_CAPNAME(le)+"的冒險隊伍.　\n"
	);
		return;
	}
	...
	tell_object(me,"你加入了"+C_CAPNAME(le)+"的冒險隊伍.　\n");
	tell_object(le, C_CAPNAME(me)+"決定加入你的冒險隊伍.　\n");
	tell_room( environment(le), 
		C_CAPNAME(me)+"加入"+CAPNAME(le)+"的冒險隊伍.　\n",
		({ me, le }) );
...
void disband_party(object le)
{
	...
	for(i = 0; i < sz; i++) {
		...
		tell_object(members[i],"你的冒險隊伍解散了.　\n");
...
void disband_member(object le, object me, int flag)
{
	...
	if(flag) {
		tell_object(me, 
			C_CAPNAME(le)+"將你從"+ to_chinese(POSS(le)) +"冒險隊伍中除名.　\n"
		);
		tell_object(le,"你將"+C_CAPNAME(me)+"從隊伍中除名.　\n");
	} else {
		tell_object(me,"你脫離"+CAPNAME(le)+"的冒險隊伍.　\n");
		tell_object(le, C_CAPNAME(me)+"脫離了你的冒險隊伍.　\n");
	}
...
void change_leader(object le, object me) 
{
	...
	for(i = 0; i < sz; i++) {
		...
		tell_object(members[i], C_CAPNAME(me)+"成為你冒險隊伍的新領袖.　\n");
	}
...
void party_list(object le) 
{
	...
	sz = sizeof(members);
	write( "你的冒險隊伍共有 " + sz + " 個成員:\n");
	write("  "+C_CAPNAME(le)+" [領袖]\n");
	for(i = 0; i < sz; i++) {
