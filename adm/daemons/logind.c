...
//  the banish daemon for use of a banished site.
    ...
    if( hibernate && time() < hibernate ) {
        write("\n\n\t咦 ? 你不是發誓說要努力用功, 在 " + ctime(hibernate) +
              " 之前\n\t都不 LOGIN 嗎 ?? 快 !! 快去看書。\n\n");
        ob->remove_user();
        return;
    }
...
因為某些原因系統無法載入你的人物資料，請用 Email 通知巫師。
...
protected void enter_world(object user)
{
    ...
    write( "目前權限：" + DOMAIN_D->query_domain_level(player) + "\n");

/*
    log_file("log_rec",extract(ctime(time()), 4, 18) +" : "+ player->query("name") +
         " as "+file_name(player)+"\n   從 " + query_ip_name(player)+
         " 連線進入東方故事. \n\n");
*/
}
...
string active_users(int irc)
{
    ...
    if( !irc )
      return "目前在線上的玩家: " + output + "\n共有: " + sizeof(who) + " 個玩家.  (上限 : "+(options["MAX_USERS"]+extra)+" 人)";
    else
