...
void create()
{
	set_short("@@query_c_name");    
   	...
   	set("value", ({0, "silver"}) );       // 野生的樹當然不值錢啊
        ...
        set("herb_file","yellow_lotus.c") ;     //提煉出來的東東的完整檔名
        set("herb_path","/d/healer/reagent/");   //提煉出來的東東的目錄
                                              //記得用完整目錄
                                              //default 是 /d/healer/plant
                                                
        set("skill_require",45) ;             //要成功辨識這種植物所須之技能
...
string query_c_name()
{
  ...
  if (this_player()->query_stat("int") < 10 ) {
    ...
    set_long("這株水生植物外形和一般的似乎有些不同, 可惜以你簡單的小腦袋瓜,\n"
             "很難說出個所以然來。\n");
    return "水生植物";
  }
  else if ( ok==1 ) {
    ...
    set_name("lotus","蓮花");
    set_long("根據你仔細鑑定的結果, 你判斷這是一株蓮花,蓮子心可做黃蓮(yellow lotus),\n"
    ...
    set("id",({"plant","tree"}) );
    set_name("a plant","水生植物");
    set_long("這棵水生植物並不普遍, 要想知道正確名稱須進一步的鑑定。\n");         
    return "水生植物";
  }
}      
