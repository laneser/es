...
{
    seteuid(getuid());
    set_name("Magic Armored Lance", "鎧魔槍");
    set_short("鎧魔槍");
    add("id", ({"lance"}));
    set_long(@C_LONG
傳說中魔族的超級戰士貝拉里爾的得意武器，槍柄上刻著鑄造這把神兵的偉大
工匠亞爾羅斯的名字，這把美麗的槍，無論從任何角度欣賞，都令人由衷讚歎
鑄造者的巧思。
C_LONG
    );
    set("unit", "把");
...
void wield(int silent)
{
    ...
    {
	tell_object(p, "這把槍的構造太複雜，你不知如何使用它。\n");
	::unwield(0);
	return;
    }
    ...
    { 
        ...
        {
            tell_object(p, "你必須卸下與鎧魔槍衝突的護具才能裝備鎧魔槍。\n");
	    ::unwield(0);
	    return;
        }
    }
    ...
    if (p->query("weapon2"))
    {
	tell_object(p, "你必須卸下所有武器才能裝備鎧魔槍。\n");
	::unwield(0);
        return;
    }
    ...
    tell_object(p, set_color("鎧魔槍呼應著你的勇氣，幻化出一套盔甲。\n\n", "HIY"));
	...
	tell_room(environment(p), s+"順利地戴上鎧魔臂。\n", p);
        ...
        leggings->move(p);
        leggings->equip(1);
        tell_object(p, "你順利地拿起鎧魔盾。\n");
	tell_room(environment(p), s+"順利地拿起鎧魔盾。\n", p);
        p->calc_armor_class();
        hands = new(ARM"hands");
        hands->move(p);
        hands->equip(1);
    ...
    tell_room(environment(p), set_color("鎧魔槍幻化出一套盔甲，順利地穿在"+p->query("c_name")+"的身上!\n", "HIY"), p);
    set("prevent_drop", 1);
    set("wielded", 1);
    p->calc_weapon_class();
}
