...
int cast(int level, string target)
{
	...
	me->set_temp("msg_stop_attack","( 你正在施法，無法攻擊。 )\n"  );
	write( set_color(sprintf("你開始念起絞心術咒文...\n"),"HIG",me));
...
{
   if( level < 3 ) {
     tell_object( caster,set_color(sprintf( "你念著咒語，召喚著邪惡的力量。\n" ),"HIG"));
	 tell_room( environment(caster), 
sprintf("%s口裡唸唸有詞，似乎正在施展某樣法術。\n",caster->query("c_name")) , caster);
   } else
     if (level < 6 ) {
     tell_object( caster,set_color(sprintf( "你念著咒語，召喚強大的邪惡力量前來。\n" ),"HIG"));
	 tell_room( environment(caster),set_color( 
sprintf( "%s口裡唸唸有詞，似乎正在施展某樣法術，你忽然感到一陣莫名的恐懼。\n",caster->query("c_name")) ,"HIG"), caster );
	} else
     tell_room( environment(caster),set_color(sprintf( "突然間一股極強大的神秘力量開始籠罩著這裡，好像有什麼事要發生了，讓你感到不寒而慄。\n" ),"HIG"));
...
void effect_one(int level, object caster, object victim)
{
	...
	if (ali > -200) {
		tell_room( environment(caster), "你念完最後的咒語，但是什麼事也沒有發生....。\n");
		return;
	}
	...
	if( !victim || !present(victim, environment(caster)) ) {
	    tell_object( caster, "你念完最後的咒語，可是敵人已經不在了，什麼事也沒發生。\n");
	    tell_room( environment(caster), sprintf("%s口裡念著奇怪的咒語，但是什麼事也沒有發生。\n",
	      caster->query("c_name")) , caster);
    ...
    if( !success_hit( 10+level*15, caster, TYPE, query_need_spell_level(level) ) ) {
      tell_object( caster, 	sprintf("你念完最後的咒語，但是%s卻沒有事的樣子，可惡！又沒擊中。\n", victim->query("c_name") ) );
	  tell_room( environment(caster), sprintf("%s口裡念著奇怪的咒語，但是什麼事也沒有發生。\n",
	    caster->query("c_name")), caster);
	  gain_spell_experience(caster, TYPE, query_gain_spell_exp(level)/2);
	  return;
...
// hit effect ....
    if( level < 3 ) {
	  ...
	  tell_object( caster,set_color( sprintf("你念完最後的咒語，看到%s露出痛苦的表情。\n",
		 victim->query("c_name") ),"HIG",) );
	    tell_object( victim, sprintf("%s喃喃地念著詭異的咒文，使你覺得心臟絞痛！\n ",
		 caster->query("c_name") ) );
	  tell_room( environment(caster),sprintf("%s喃喃地念著詭異的咒文，你看到%s露出痛苦的表情。\n",
	     caster->query("c_name"), victim->query("c_name")) ,  ({caster,victim}));
    } else {
	  tell_room( environment(caster),set_color( sprintf( "%s身子為之一震，搖晃了幾下，臉上露出了痛苦的表情。\n",
...
// hit effect ....
    if( level < 3 ) {
/*	  tell_object( caster, sprintf(bold("你念完最後的咒語，但卻忽然覺得心臟絞痛！\n") ) );
*/
	  tell_object( caster,set_color( sprintf("你念完最後的咒語，但卻忽然覺得心臟絞痛！\n"),"HIR" )) ;
	  tell_room( environment(caster),sprintf("%s喃喃地念著詭異的咒文，卻忽然露出痛苦的表情。\n",
	     caster->query("c_name")) , caster);
    } else {
	  tell_room( environment(caster),set_color( sprintf( "%s身子為之一震，搖晃了幾下，臉上露出了痛苦的表情。\n",
...
void effect_all(int level, object caster, object targ)
{
    ...
    tell_room( environment(caster),set_color(sprintf( " 突然，你看見一團黑暗的物體快速的膨脹爆開，向你襲來。\n" ),"HIG",));
    tell_room( environment(caster),set_color(sprintf(  "\n你只見眼前一暗，腦中短暫的空白之後，隨即感到全身的血管好像都爆開了。\n\n" ),"HIG"));
