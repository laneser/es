...
void create()
{
    ...
    seteuid(getuid());
        ...
        set_short("牧師諾倫");
        set_long(@TEXT
在你面前的是一個身形高大, 看起來跟本不像牧師的牧師. 多年的冒
險經驗告訴你, 他是因為要躲某人的追殺才在這裡當牧師的.
不過話又說回來... 既然他在這裡了, 你要結婚也可以找他主持.
...
int perform_marriage()
{
    ...
    if (!ob) {
        command("say 哈... 笑死人了, 新郎臨陣脫逃啊?!?");
        command("shake");
        command("say 罷了罷了... 下次吧!\n");
        set("husband");
        set("wife");
        set("witness");
        set("wait");
    ...
    command("say "+n1+", 你願意娶"+n2+"為妻, 並答應要好好照顧她,");
        command("say 不論經歷多少 reboot, crash 或 lag, 並無條件地幫她接雷,"); 
        command("say 賺錢供她花, 保護她不受傷害嗎?");
    tell_object(ob, "諾倫回頭對你說: 請回答(answer) Yes 或 No.\n");
...
int perform_marriage2()
{
    ...
    command("say "+n2+", 你願意嫁給"+n1+", 陪他渡過 reboot, crash 或 lag.");
    command("say 幫他花錢, 收集裝備嗎?");
    tell_object(ob, "諾倫回頭對你說: 請回答(answer) Yes 或 No.\n");
...
int perform_marriage3()
{
    ...
    if ((!ob) || (!ob1) || (!ob2)) { //anyone of them is missing
        command("say 咦... 人不見了還結什麼婚啊.. 罷了罷了..");
        return 1;
    }
    if (wizardp(ob)) { // witness is a wizard
    command("smile");
    command("say 那麼在公證人"+n3+"及大家的面前, 我宣佈你們正式成為夫妻!");
    command("shout "+n1+" 跟 "+n2+" 在巫師的祝福下結為夫妻了, 大家祝福他們!!!!!");
    ...
    command("grin");
    command("say 照 ES 的規矩, 沒有巫師當公證人的婚禮都算私奔.");
    command("say 不過呢, 既然你們這麼相愛, 等不及了...");
    command("grin");
    command("say 那麼在公證人"+n3+"及大家的面前, 我宣佈你們正式成為夫妻!");
...
int wait_answer(string str)
{
    ...
    command("?");
        ...
        {
    tell_object(this_player(),"你說道: 我願意!\n");
    tell_room(environment(this_player()),(string)this_player()->query("c_name")+
    "說道: "+"我願意!\n",({this_player(), this_object()}));
    command("slap "+(string)this_player()->query("name"));
    command("say "+capitalize((string)this_player()->query("name"))+", 我不是問你!");
        return 1;
        }
    if (!str || member_array(lower_case(str), ({"yes","y"})) == -1)
        {
        tell_object(this_player(),"你想了一會, 還是不要吧.\n");
        tell_room(environment(this_player()),
        (string)this_player()->query("c_name")+"面有難色, 說道: "+
        "嗯... 這個... 我想還是算了.\n",({this_player(), this_object()}));
        command("kick "+(string)this_player()->query("name"));
        command("say 決定好再來! 沒事不要煩我.. 小心我 Z..A..P 你喔!");
        command("grin");
...
int perform_divorce()
{
      command("say 你們真的要離婚嗎? ");
      command("say 你們要慎重考慮一下, 到底是同意(agree),還是拒絕(refuse)?");
...
int do_shake(string str)
{
    ...
    if(n1!=me->query("name") && n2!=me->query("name"))
    {
       ...
       command("say "+(string)me->query("c_name")+", 我不是問你!");
       return 1;        
    }
    if(n1==lower_case(me->query("name")) ) 
    {
       command("say "+capitalize(n1)+" 不同意離婚, 離婚失敗!");
    }
    else
    {
       command("say "+capitalize(n2)+" 不同意離婚, 離婚失敗!\n");
    }
    command("kick");
    command("spit");
    command("say 沒準備好就別來煩我, 他XX的!");
    divorce_end();
    return 1;
}
int do_nod(string str)
{
    ...
    if(n1==me->query("name") ) 
    {
       command("say "+capitalize(n1)+"同意了離婚!");
       if( query("female_agree") <= 1 )
         command("say 那麼你呢, "+capitalize(n2)+"?");
       set("male_agree",2);
    }
    if(n2==me->query("name") ) 
    {
       command("say "+capitalize(n2)+"同意了離婚!");
       if( query("male_agree") <= 1)
         command("say 那麼你呢, "+capitalize(n1)+"?");
       set("female_agree",2);
    }
    if(query("male_agree")>1 && query("female_agree")>1)
    {
       ...
       if(!ob1 || !ob2 || !wiz)
       {
          command("say 人不全，無法繼續下去!\n");
          divorce_end();
          return 1;
       }
       command("shout "+capitalize(n1)+"和"+capitalize(n2)+"離婚了!");
       if((string)wiz->query("name")=="moon")
command("shout "+"是不是"+capitalize(n2)+"迷上了風流倜儻的"+wiz->query("c_name"));
       else
command("shout "+wiz->query("c_name")+"為什麼這麼主動的做見證人, 是不是做了第三者!");
