...
void create ()
{
	...
	set_short( "金神蓐收");
	set_long(@CLONG
蓐收全身長得漆金也似的，一對眼睛大如銅鈴，相貌極有威儀。蓐收原是輔佐西天
帝少昊的，掌管著四季中的秋天。由於蓐收是屬於五行中的西金，不免有些穿鑿附
會之說，一但遇見他，總會財富滾滾。	
CLONG
	);
	set("race","god");
...
int my_tactic()
{
    ...
    tell_room(environment(),
      "蓐收手中的璇天尺不停地旋轉，變成一團栲栳大的光圈激射而出！\n",victim);
    tell_object(victim,
      "\n蓐收手中的璇天尺不停地旋轉，變成一團栲栳大的光圈 ...\n\n那團光圈忽然激射出萬丈豪光射向你的全身!！\n\n\n");
    tell_object(this_object(),sprintf(
      "\n你手中的璇天尺不停地旋轉，變成一團栲栳大的光圈 ...\n那團光圈對著%s激射出萬丈豪光!！\n\n\n"
      ,victim->query("c_name")));
...
void die()
{
   ...
   max=sizeof(atk);
   tell_room(environment(),"蓐收臨死前懷著深沈的惡意道：你們這些人會受到詛咒的！\n");
   for ( i=0;i<max;i++ ) 
      if ( userp(atk[i]) ) 
         damn_player(atk[i]);
   ::die();
