...
nomask int cmd_hook(string cmd)
{
	string file;
	string verb;
	int foo ;
...
/*
nomask  int stack_cmd_hook(string cmd)
{
	string file;
	string verb, arg;
	int foo ;
...
int quit(string str)
{
	...
	if( str )
		return notify_fail( "要離開遊戲，打 quit 就好了。\n");
	if ( wizardp(this_object()) && !this_object()->query("ok_ip"))
		return notify_fail( "身為巫師，請先設好 OKIP 再 quit 以確保安全 !!\n");
// If the #define is on, then save their location for starting next time.
	...
	if( wizardp( this_object() ) )	{
		...
		if( file_size( quit_script ) > 0 ) {
			if (in_de_quit_script++)
				write( "喂！別在 .quit 裡面放 \"quit\" 指令，呆子。\n");
			else
	...
	if( this_object() && visible(this_object()) && environment() )
		tell_room( environment(),
			query("c_name") + "決定暫時離開東方故事，回到可怕的現實世界去了。\n"
			, this_object()
		);
...
void setup()
{
	...
	if( link_data("last_on") )
		write( "\n上次連線時間:  " + ctime(link_data("last_on")) + " (從 " +
			link_data("ip") + ")\n\n");
...
void complete_setup (string str)
{
	...
	object ghost;
	string aclass, start_room, student_time;
/*
...
/*	if (!timetemp1){
           tell_object(this_object(),"你的人物資料已經損壞，請用其他人物通知大神或巫師 !!\n");
...
//	set("wreply", 0);
	...
	if( !start_room || !stringp(start_room) ){
		aclass = this_object()->query("class");
		if ( member_array( aclass, CLASS_NAMES ) == -1 ) {
			tell_object(this_object(),"你的人物資料已經損壞，請用其他人物通知大神或巫師 !!\n");
			remove();
			return;
		}
		start_room = call_other( GUILD_MASTER(aclass), "query_guild_room" );
	}
...
of users, or leave it commented out...
	...
	if( visible(this_object()) )
		tell_room( environment(),
			   (string)this_object()->query("c_name") + "連線進入了東方故事。\n",
			  this_object() );
	...
	if( objectp(statue) && type == "linkdead" ) {
		if( environment(statue) )
			tell_room( environment( statue ),
				user->query("c_name")+
				"斷線太久，至高無上的神決定把這傢伙踢回現實世界。\n"
				"你聽到「轟隆」一聲，不知何處落下一道閃電把"+statue->query("short")+
				"化為灰燼！\n"
				, statue
			);
			statue->remove();
...
string query_short()
{
	...
	if( query("name") == "noname" )  return "無名氏";

	if( !interactive(this_object()) )
		return (query("title") + " [斷線中...]");

	if( query("inactive") )
		return query("title") + " [停止活動]";
/*
