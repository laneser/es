...
int cmd_diagnose(string arg)
{
        ...
        if ( !arg || arg == "" ||
             !(dest = present(arg,environment(this_player()))) ) 
                return notify_fail("你要幫誰診斷 ?\n");
 
        if ( ! living(dest) )
                return notify_fail("這個東西沒有生命。\n");
        if ( (skill = this_player()->query_skill("diagnose")) == 0 ) 
                return notify_fail ("你並沒有學過診斷的技能。\n");
        ...
        if(!max_hp) return notify_fail("這個生物有問題,請連絡巫師。\n") ;
        ...
        if( dest != this_player())
          {
            ...
            return notify_fail("要知道自己的狀況, 打 Score 就好啦 !!\n") ;          
 
        write( sprintf( "%s現在的體力約剩下%d %% , 目前的身體狀況 %s \n"
                        "正在作用中的針灸效果: %s。\n"
