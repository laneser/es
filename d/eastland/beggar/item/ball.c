...
void create()
{
        ...
        set_short( "舍利子" );
        set_long(@C_LONG
這是一顆白色的小小圓球。據說舍利子是修行高深的有道和尚死後火化才會遺留
下來的東西。一般而言, 舍利子含有死者一生的精華修為,如果能得到箇中秘密, 
或許能增強你的修為也說不定喔!!你可以試著持有它( hold )看是否有什麼收穫。
C_LONG        
        );
        set( "unit","個" );
        set( "weight", 5 );
...
void run_out()
{
   ...
   mapping skills ;
     ...
     for(i=0 ; i <sizeof(skill_name);i++){
         ...
         mod_val=(mod_val/20);
     if(!(mod_val))
       tell_object(owner,"你的技能檔案似乎不正常,請速洽巫師解決。\n") ;
     else
