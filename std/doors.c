   ...
   object ob;
	...
	if (link && link != "none") {
		if (ob = find_object((string)query("exits/"+dir))) {
			...
			switch (door_stat) {
				case "open" : {
					tell_room (ob, to_chinese(ob_dir)+"邊的門突然「刷」的一聲推開，差點撞到你的鼻子.\n") ;
					break ;
				}
				case "locked" : {
					tell_room(ob,to_chinese(ob_dir)+"邊的門「喀啦」的一聲，好像被鎖上了.\n") ;
					break ;
				}
				case "closed" : {
					if (old_stat=="locked") {
						tell_room (ob, to_chinese(ob_dir)+"邊的門發出「喀嚦」的一聲，好像鎖被打開了.\n") ;
					} else
						tell_room (ob, to_chinese(ob_dir)+"邊的門突然「碰」的一聲關上，把你嚇了一大跳 .\n") ;
					break ;
				}
			}
		}
