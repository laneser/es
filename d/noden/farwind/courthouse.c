...
void create()
{
	...
	set ("short", "法院");
	set ("long", 
			"你現在來到遠風鎮的法院。法院裡的法官可以為你和你的愛人證婚，不\n"
		"過別想在這裡離婚，遠風鎮的人將婚姻視為一種神聖不可侵犯的終身誓約，\n"
		"想要結婚的人，請 help marriage 以得到進一步的說明。\n");
...
int married_to_whom (string str)
{
	...
	if (spouse=="none") {
		write( capitalize(str)+" 現在還是單身，沒有結過婚。\n");
		return 1 ;
	}
...
int get_married (string str)
{
	...
	return notify_fail( "和自己結婚？好主意，不過在這裡行不通。\n");

        if ( (string)this_player()->query("class") == "monk" )
	return notify_fail(
	"法官開口罵道：「你這不守清規的臭和尚 !! 想誘拐良家婦女嗎？」\n" );
	...
	if( this_player()->query("spouse") )
		return notify_fail("你已經結過婚了！\n") ;
	if( find_spouse(capitalize(str))!="none" )
		return notify_fail( capitalize(str) + " 已經結過婚了 ....。\n");
	if( !find_player(str) || !present(str, environment(this_player())) )
		return notify_fail("你的結婚對象不在這裡 ....。\n");
	if (numpending==0) {
		add_pending(str) ;
		return 1 ;
	}
...
int perform_marriage(string s1, string s2)
{
	...
	if (!spouse)
		return notify_fail( 
			"你的結婚對象已經離開了「東方故事」，無法進行婚禮。\n");
	spouse->set( "spouse", s1);
	this_player()->set( "spouse", s2);

	tell_object(spouse, 
		"恭喜！雙方情投意合，法官現在正式宣佈你和 "+s1+" 成為合法夫妻！\n");
	write( 
		"恭喜！雙方情投意合，法官現在正式宣佈你和 "+s2+" 成為合法夫妻！\n");
	shout("你聽到遠風鎮的教堂鐘聲歡欣地當當齊響 .... \n"+
	      "路人們都歡欣唱道：「恭喜這對新人 !! 祝福 "+capitalize(s1)+" 和 "+capitalize(s2)+
	      " 永遠幸福快樂 !!」\n"); 
