...
int cmd_fist(string str)
{
	...
	if( str=="now") {
		...
		if( !fist_now || !sscanf(fist_now, FIST_DIR+"%s", fist_now) )
			write("你目前沒有打算使用任何拳法.\n" );
		else write("你隨身使用的拳法是: "+set_color(to_chinese(fist_now), "HIC")+"。\n");
		return 1;
	} else if( str == "none" ) {
		write("你決定不使用的基礎拳法。\n");
	...
	if( member_array(str, fist) == -1 )
		return notify_fail("有這種基本拳法嗎? \n");

	know = (string *)me->query("monk_gonfu/fist");
	if( !know || !sizeof(know) || member_array(str, know) == -1 )
		return notify_fail("你會這種基本拳法嗎? \n");

	write("你隨身使用的拳法現在是: "+set_color(to_chinese(str),"HIC")+"。\n");
...
{
		write( @C_HELP
指令格式: fist <少林基本拳法名稱> | <none>
這個指令讓你用來施展拳法。你可以用 gonfus 指令看你目前已經學會的招式列表。
