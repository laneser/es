...
void effect(int level, object caster, object victim)
{
	...
	if( !success_hit( 10+level*10, caster, TYPE, query_need_spell_level(level) ) ) {
	  tell_object(caster, 
	    sprintf("你將蓄滿能量的手向%s一指，「颼」的一聲結果射歪了！\n",
			victim->query("c_name") ));
	  tell_room( environment(caster), 
		sprintf("忽然%s用手向前一指，「颼」的一聲不知什麼東西射了出來可是沒打中任何人！\n",
		  caster->query("c_name") )
		,caster
	  );
	  gain_spell_experience(caster, TYPE, query_gain_spell_exp(level)/2);
	...
	switch( level ) {
		case 0..7:
			cmsgc = "你將蓄滿能量的手向%s(%s)一指，「颼」的一聲射出一股無形的能量！\n";
			cmsgv = "%s(%s)揚手向你一指，「颼」的一聲不知道什麼東西射中了你！\n";
			cmsgo = "忽然%s(%s)用手向%s一指，「颼」的一聲，%s(%s)臉上露出痛苦的表情。\n";
			break;
		case 8:
			cmsgc = "你將蓄滿能量的手向%s(%s)一指，射出了一股奇異的光芒！\n";
			cmsgv = "%s(%s)揚手向你一指，一股奇異的光芒射中了你！\n";
			cmsgo = "忽然%s(%s)用手向%s一指，一股奇異的光芒射中了%s(%s)。\n";
			break;
		default:
			cmsgc = "你將蓄滿能量的手向%s(%s)一指，「颼」的一聲射出一股無形的能量！\n";
			cmsgv = "%s(%s)揚手向你一指，「颼」的一聲不知道什麼東西射中了你！\n";
