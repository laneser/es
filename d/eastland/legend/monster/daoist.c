#include "../legend.h"

inherit MONSTER;
void create ()
{
        ::create();
        set_level(19);
        set_name( "yellow daoist", "茅山道士 黃天化" );
        add("id", ({"daoist"}) );
        set_short( "茅山道士 黃天化");
        set_long(@C_LONG
你眼前所見是位穿著喇禢，混身骯髒又不修邊幅的茅山道士。但是，你可別小看他
，因為他是現今茅山派第三十八代的掌門人，不但武功高，功力更是深厚無比，畢
生伏魔無數，頗贏得江湖人士的愛載。黃天化最近聽說此處有妖魔鬼怪出現，特來
查看一番。黃天化受傷，愁眉深鎖，好似遇到了什麼麻煩。
C_LONG
);
        set ("gender", "male");
        set ("race", "human");
        set( "alignment", 400 );
        set_natural_armor( 35, 48 );
	set_natural_weapon( 53,12,36);
        set_perm_stat( "str", 25 );
        set_perm_stat( "kar", 25 );
        set_perm_stat( "int", 28 );
        set_perm_stat( "dex", 25 );
        set_skill( "dodge", 100 );
        set_skill( "parry", 80 );
        set_skill( "axe",75);
	set( "max_fp",500);
	set( "force_points",500);
        set( "hit_points", 900 );
        set( "max_hp", 1000000 );
        set("conditions/_heal_hp",({ 10000 , 1 }) ) ;
        set( "special_defense", ([ "all": 50,"none":40,
                "monk":40,"scholar":20]) );
        set( "stun_difficulty",50);
	wield_weapon( LWEAPON"mahogany_axe.c" );
	equip_armor( LARMOR"daoist_cloth" );
        set( "chat_chance", 10 );
        set( "att_chat_output", ({
                "黃天化喊道：「惡賊！還不退 !!」。\n",
	}) );
        set( "inquiry", ([
             "trouble":"@@ask_trouble",
             "定海珠":"@@ask_ball",
             "無支祈":"@@ask_mob",
        ]) );
        set( "tactic_func", "my_tactic" );
}
void ask_trouble(object asker)
{
write(@C_LONG
黃天化嘆道：唉近來聽說此處有上古神獸「無支祈」，特來收伏它。但卻敵它不過，
　　　　　  受了極大的傷，並且遺失了一顆把它引出的媒物－「定海珠」，希望
　　　　　　少俠可以找回來，並收拾「無支祈」。
C_LONG
      );
}           
void ask_ball(object asker)
{
write(@C_LONG
黃天化道：你問「定海珠」?
          哦,那是一顆藍色的原球，你只需在湖邊 ( roll sea-ball ) 就可以
          把「無支祈」給引出來，但請小心，因為....
C_LONG
      );          
}
void ask_mob(object asker)
{
write(@C_LONG
黃天化道：「無支祈」本是上古神獸，它具有復活的本能，得趁它奄奄一息時用
　　　　　  大鐵鎖鎖住方能制伏它。
C_LONG
     );
}
int my_tactic()
{
   int i,num;
   object victim,env,*all,help;
   
   if ( !victim=query_attacker() ) return 0;
   if ( random(10)>2 ) return 0;
   env=environment(this_object());
   all=all_inventory(env);
   num=0;
   for (i=0;i<sizeof(all);i++)         
     if ( all[i]->query("paperman") ) num+=1; 
   if ( num>2 ) return 0;  
   tell_room(env,"\n黃天化急忙從懷中掏出符咒，向天空一丟，化為一堆兇惡紙人！\n\n");    
   for (i=0;i<(3-num);i++) {
      help=new(LMONSTER"paperman");
      help->move(env);
   }
   return 0;        
}
void die()
{
   int i;
   object env,*all;
   env=environment(this_object());
   all=all_inventory(env);
   for (i=0;i<sizeof(all);i++)
     if ( all[i]->query("paperman") ) all[i]->remove();
   ::die();
}