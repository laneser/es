...
#define UNITS ({"頂","件","條","雙","條","雙","面","條","個","個" })
#define ADD_NAME1 ({"皮帽","皮甲","皮臂環","皮手套","皮褲","皮靴","皮盾","皮披風","皮馬鞍","綁尾"})
#define ADD_NAME2 ({"皮頭巾","皮套體","皮護肘","皮護手","皮裙","皮鞋","皮盾","皮披肩","皮坐墊","皮尾甲"})
...
void create ()
{
        ...
        set_long(
@C_LONG
一位正在辛苦工作的女裁縫，她的主要工作是把毛皮硝製成各種防具
C_LONG
);
        set ("gender", "female");
        set ("race", "human");
...
int do_choose(string arg)
{
 ...
 for( i=0; i<sizeof(allocate(10)); i++ ) {
     ...
     tell_room(environment(this_player()),
             "女裁縫師開始把皮剪成許多小塊，然後很快的縫合起來。\n");
     command("say 這就是您要的防具，歡迎光臨，下次再來。");
...
MISSION
        ...
        tell_object(me,
@MISSION
女裁縫師說道: 喔，你好，你想要製作些什麼防具呢??
現在我能製作的防具有下面幾種:
head(頭部)，body(身體)，arms(手臂)，hands(手部)，legs(腿部)
feet(腳部)，shield(盾牌)，cloak(披風)，saddle(馬鞍)，tail(尾巴)
你可以選擇(choose)其中的任何一種。
MISSION
	);
