...
//	seteuid(geteuid(owner));
	set_short( (string)owner->query("cap_name")+"'s golden credit card",
		(string)owner->query("c_name")+"的東方故事金卡");
	set_long(
		...
		"這是一張屬於"+(string)owner->query("c_name")
		+"的黃金信用卡，你可以用它在東方故事中的任何銀行存、提款。\n"+
		"用 balance 指令可以查看你目前的帳戶金額。\n") ;
	::move(owner) ;
