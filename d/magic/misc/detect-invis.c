...
int cast(int level,string target)
{
   ...
   if (target =="NONE") dest=me;
           ...
           tell_object( me, "要對誰使用偵測隱形術?\n" );
         return 0;
    }

    if( dest->query_temp("effect/detect-invis") ) {
	if (dest == me)
      	tell_object( me, "你上一個偵測隱形術還沒消失呢! 別浪費法力了。\n" );
	else
      	tell_object( me, "他上一個偵測隱形術還沒消失呢! 別浪費法力了。\n" );
      return 0;
    }
	if (dest == me) 
	write( set_color("你施展偵測隱形術，感到你的眼睛為之一亮。\n","HIY"));
	else{
	tell_object(me,"你施展偵測隱形術，"+dest->query("c_name")+"的眼睛發出異樣的光芒。\n");
	tell_object(dest,set_color("你感到你的眼睛為之一亮。\n","HIY"));
	}
...
{
    if( !caster ) return;
    tell_object( caster , set_color("你感覺到你的眼力又恢復正常了。\n" ,"HIW"));
