// This is a spell to let caster can see other hidden players	by Yueh@East Story 1995/3/11
...
int cast(int level,string target)
{
	...
	if( (int)me->query_skill("misc") < query_need_skill(level) ) {
	  tell_object( me, "你的雜項魔法技能不足以使用這等級的魔法! \n" );
	  return 0;
	}
   if (target =="NONE") dest=me;
    else if ( !(dest = present (target, environment (me) ))) {
           tell_object( me, "要對誰使用偵測躲藏術?\n" );
         return 0;
    }

    if( dest->query_temp("effect/detect-hide") ) {
	if (dest == me)
      	tell_object( me, "你上一個偵測躲藏術還沒消失呢! 別浪費法力了。\n" );
	else
      	tell_object( me, "他上一個偵測躲藏術還沒消失呢! 別浪費法力了。\n" );
      return 0;
    }
	if (dest == me) 
	write( set_color("你施展偵測躲藏術，感到你對周遭的敏感度提高了。\n","HIY"));
	else{
	tell_object(me,"你施展偵測躲藏術，"+dest->query("c_name")+"的眼睛發出異樣的光芒。\n");
	tell_object(dest,set_color("你感到你對周遭的敏感度提高了。\n","HIY"));
	}
	tell_room( environment(me), 
		me->query("c_name") + "喃喃地念了一段咒文，" 
		+ dest->query("c_name") + "的眼睛發出異樣的光芒。\n" ,
		({me,dest})
	);
    dest->set_temp("detect_hide", 1);
    dest->set_temp("effect/detect-hide", 1);
...
{
    if( !caster ) return;
    tell_object( caster , set_color("你感覺到你對周遭的敏感度又恢復正常了。\n" ,"HIW"));
    caster->set_temp("detect_hide", 0);
    caster->set_temp("effect/detect-hide", 0);
}
