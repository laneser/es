...
{	int *savedata,day,hour;
	...
	if( !sizeof(top_list) ) return "探險度排行榜目前是空的。\n";
	
	total_points=EXPLORE_D->query_total_explore("noden");
	if (total_points < 1) return "Abort program !!(系統尚未設定任何探險度或有其他錯誤發生)\n";
	msg ="\n諾頓大陸探險度排行榜前 " + sizeof(top_list) + " 名:\n";
	msg += sprintf(" 名次  玩家姓名           等級          上線時間               探險度\n");
	for(i=0; i<sizeof(top_list); i++) {
		...
		msg += sprintf("  %2d. %20-s %2d %10d 天 %2d 小時 %13d.%1d%1d%%\n",i+1,
	...
	if (total_points < 1) return "Abort program !!(系統尚未設定任何探險度或有其他錯誤發生)\n";
	msg += "\n東方大陸探險度排行榜前 " + sizeof(top_list1) + " 名:\n";
	msg += sprintf(" 名次  玩家姓名           等級          上線時間               探險度\n");
	...
	if (total_points < 1) return "Abort program !!(系統尚未設定任何探險度或有其他錯誤發生)\n";
	msg += "\n死亡大陸探險度排行榜前 " + sizeof(top_list2) + " 名:\n";
	msg += sprintf(" 名次  玩家姓名           等級          上線時間               探險度\n");
