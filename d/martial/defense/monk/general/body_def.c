...
// 原本這項特攻是指回復 hp , 但因為似乎與金剛不壞體沒有什麼關連,
// 而關閉. 現在新寫內力反震為特攻, 依據 player 的內力為根基來決定
// 內力反震的大小.....
// 依據目前 player 的 inner-force skill<=95 & force_effect<=4
// 0 < return_dam < 53    0 < got_heal < 25
// 以上兩值皆與 body-def 的 lv 成直線相關....

{
    ...
    tell_object( ob1,
       sprintf("你迅速地在身周鼓動氣流護體，將%s的攻擊反震回去，自己卻也受到內力的激湯而連退三步。\n",
           victim->query("c_name") )
    );
    
    tell_room( environment(ob1),
       sprintf("\n%s的臉上突然閃過一陣紫氣，金剛不壞神功的奧妙心法似乎發揮了難以想像的威力！\n\n",ob1->query("c_name")),
    ({ victim, ob1 }) );
    
    ob1->block_attack(4);

    tell_object( victim,
       sprintf("\n你突然感到不對，%s強大的內力反震使你氣血翻湧，一時動彈不得。\n",
           ob1->query("c_name") ) 
    );
