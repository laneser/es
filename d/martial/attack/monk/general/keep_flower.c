...
// 拈花指的特攻為點穴, 原出現皆為 block_attack(4), 今更動為
// block_attack(lv/8 + random(lv/16)) 。
// 從最少 block_attack(0) 到 最多 block_attack(6)
// 與 keep_flower lv 成正相關....
...
tell_object( victim,
 (string)ob1->query("c_name")+"使出『拈花指』點了你的穴道！\n\n");

tell_room( environment(ob1), 
 "\n\n"+(string)ob1->query("c_name")+"突然運氣一指，點住了" + (string)victim->query("c_name") + "的穴道！\n",
({ victim, ob1 }) );
...
write(
"\n\n你的拈花指一彈，點住了" + (string)victim->query("c_name") +"的穴道！\n\n" );
...
string *attack_msg = ({
	...
	"%s一指『靜極便覺』彈向%s",
	"%s臉露微笑，一點指力飛向%s",
	"%s雙眉微蹙，一記捻花指絕學『諸幻盡滅』使出，大股指力卷向%s",
	"%s神色溫和，一招『無始無名』使出，一道指力攻向%s",   
	"%s態度平和自得，一記『花落蓮成』，幾點內勁擊向%s",
	"%s雙目微閉，一股強勁指力攻向%s",
	"%s運氣於指，使出最強的『圓悟涅盤』一連三指挾強勁內力襲向%s",
});
