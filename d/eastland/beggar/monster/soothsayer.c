...
void create()
{
    ...
    set_short( "江湖術士 神算小瓜");
    set_long(@LONG
神算小瓜看起來約莫四、五十歲年紀,他過目不忘更生得一對天眼通。他生平鑽研
卜筮之學,可謂上通天文下通地理, 凡是什麼易經之類的對他而言簡直是小兒科。
他鐵口直斷,鮮有失誤,所以很多人來求他開一金口但往往無功而返,因為他總是推
說無緣。他最近醉心於陣法之學,如果你能投他所好, 說不定他幫你算一段你就獲
益非淺。神算小瓜也喜歡熱鬧的地方,一聽到丐幫有個祭祖大典就跑來, 不知道他
是不是別有企圖。
LONG
            );
    set("unit","位");
...
void make_angry(object obj)
{
   object ob1,ob2,ob3,*all,env;        
    ...
    tell_room(env,C_NAME(obj)+
      "口裡唸唸有詞:「神恩如海天威如嶽繫系矩忙羅......」\n"+
      C_NAME(obj)+"大喝一聲從背後飛出四口飛劍,順勢躍起抓起一柄 !!\n\n\n\n");
...
int my_tactic()
{
     ...
     case 2:
               tell_room(environment(this_object()),
                  C_NAME(this_object())+"眼見勢力孤單,於是施展兩儀微塵陣\n");
               command("embattle two at "+name); 
            return 0;
     case 3:
               tell_room(environment(this_object()),
                  C_NAME(this_object())+"喝道:「看天地人三才陣」!\n");
               command("embattle three at "+name);
            return 0;
     case 4:
               tell_room(environment(this_object()),
                  C_NAME(this_object())+"施展四象璇璣陣\n");
    ...
    if ( victim->query("npc") ) {
        tell_room( environment(this_object()),
           this_object()->query("c_name")+"招來一道閃電把"
           +victim->query("c_name")+"化為灰盡....\n");
           victim->remove();
    }
    return 0;
}
