...
void create()
{
	set_name("magic cube", "魔法方塊");
	add( "id", ({ "cube" }) );
	set_short("魔法方塊");
	set("long", "@@query_long");
	set( "extra_look", "$N身旁飄浮著一個發著奇異光芒的魔法方塊。\n");
	set("weight", 0);
	set("weight_apply", 0 );
	set("max_load", 400);
...
int help ()
{
return notify_fail( "指令格式: transfer <東西> into cube \n指令格式: transfer <東西> from cube \n");
}
...
int transfer_into (string this)
{
	...
	if(res == MOVE_OK) {
		...
		write( "你口中念著咒語，你手上的" + c_word + "漸漸消失在魔法方塊裡。\n");
		tell_room( environment(this_player()), 
			this_player()->query("c_name") + "口中念著咒語，他手上的" + c_word + "漸漸消失在魔法方塊裡。\n",
			this_player());
...
// Check for the form "transfer thing from sack".
	...
	if (this != "all") {
		...
		if( res == MOVE_OK ) {
			write ( "你口中念著咒語，從魔法方塊中飛出"+c_word+"漸漸成形在你手裡。\n");
			tell_room( environment(this_player()), 
				this_player()->query("c_name")+"口中念著咒語，從魔法方塊中飛出"+c_word+"漸漸成形在他手裡。\n",
					this_player() );
			this_player()->add("spell_points", -sp );
			gain_spell_experience(this_player(), TYPE, G_EXP);
			return 1;
...
string query_long()
{
	...
	desc = "這是一個飄浮在空中發著奇異光芒的魔法方塊，似乎可以儲存不少東西。\n";
