   	...
   	string short,unit;
	    ...
	    if(is_log) log_file("GIVE", extract(ctime(time()), 4, 15)+"  "+
            ...
            if( !(short = (string)ob->query("short")) ) short = "某物";
                        if( !(unit = (string)ob->query("unit")) ) unit = "個"; 
                write(
                    sprintf("你給%s一%s%s。\n",
                ...
                write(target->query("c_name")+"拿不動了。\n");
                return 1;
} 
...
這個指令將給生物物品，但這生物必須在同一場景.

例子: give all to echobomber(將身上全部物品給echobomber) ,
      give 100 silver to kyoko(將身上100silver給kyoko) .
