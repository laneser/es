...
/*
	body->set_c_verbs( ({ "%s對準%s一記直拳", "%s猛踹%s一腿",
		"%s揮拳攻擊%s", "%s伸手抓向%s", }) );
...
int do_bloodsuck(string arg)
{
	...
	if( !arg || arg == "" ) 
		return notify_fail("你要吸誰的血？\n");

	me = this_player();
	dest = present( arg, environment(me) );
	if( !dest || !living(dest) )
		return notify_fail("這裡沒有這種生物。\n");

	if( dest->query("no_attack") )
		return notify_fail("你不能吸這傢伙的血。\n");

        if ( userp(dest) && ((int)dest->query_level()<5) )
                return notify_fail("你不能欺負低等級玩家 !!\n");
	if ( userp(dest) && ((int)me->query_level()<5) )
		return notify_fail("翅膀都還沒長硬,就想PK人?\n");
	
	if( dest->query_temp("be_sucked"))
		return notify_fail("這傢伙根本不讓你靠近，你沒有機會吸血。\n");

	if( userp(me) && me->query_temp("bloodsuck") )
		return notify_fail("嘿! 吃太飽不好哦..總要消化一下吧。\n");

	write(sprintf("你張嘴往%s的脖子咬去，",
			dest->query("c_name")));

	tell_room( environment(me), 
		sprintf("%s突然張嘴咬向%s的脖子，", 
	...
	if ( random((int)me->query_level()*2) < (int)dest->query_level() ) {
		tell_room(environment(me),set_color("但是被發現了 !!\n","HIY"),({}));
		me->set_temp("msg_stop_attack","(你現在手忙腳亂，無法攻擊 ！)\n");
		return 1;
	}

	tell_room(environment(me),set_color("開始用力的吸血 !!\n","HIR"),({}));
	me->set_temp("msg_stop_attack","(「吸血皇帝大」，其他事待會兒再說！)\n");
...
void stop_suck(object me, object victim)
{
	...
	tell_object( me, 
		"你愉快地舔了舔沾滿鮮血的嘴唇，一股滿足感使你恢復了一些體力！\n");

	tell_room( environment(me), 
		sprintf("%s愉快地舔了舔沾滿鮮血的嘴唇，露出一個滿足的表情。\n",
			me->query("c_name")), me );
