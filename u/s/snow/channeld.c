...
varargs int do_channel(object me, string verb, string arg, int emote)
{
            ...
            if(!wiz)
              {              
                if(me->query_level() < channels[verb]["lv_lim"])
                  {  return notify_fail("你的等級太低,不能使用這個頻道。\n") ; }
                if(me->query("talk_points")<channels[verb]["tp_cost"])
                  {  return notify_fail("你突然發現你說不出聲音來。\n") ; }
	...
	message( "channel:" + verb,
		sprintf( emote ? channels[verb]["msg_emote"] : channels[verb]["msg_emote"], who, arg ), ob );
