...
varargs string get_list(int flag)
{
	...
	if( !sizeof(top_list) ) return 	"魔法等級排行榜目前是空的。\n";
	if (flag==0){
	size_list=sizeof(top_list0);
	msg = sprintf("魔法等級排行榜前 %d名:\n", size_list) ;
	msg += sprintf(" 名次  玩家姓名      等級     上線時間        綜合   元素    黑    雜項\n");
	for(i=0; i<size_list; i++) {
		savedata = levels0[top_list0[i]];
		day = savedata[5]/86400;
		hour = savedata[5]%86400/3600;
		msg += 
			sprintf("  %2d. %15-s %2d %6d 天 %2d 小時 %6d %6d %6d %6d\n",i+1, 
	...
	msg = "綜合魔法等級排行榜前 " + size_list + " 名:\n";
	msg += sprintf(" 名次  玩家姓名           等級          上線時間                    魔法等級\n");
	for(i=0; i<size_list; i++) {
		savedata = levels[top_list[i]];
		day = savedata[2]/86400;
		hour = savedata[2]%86400/3600;
		msg += 
			sprintf("  %2d. %20-s %2d %10d 天 %2d 小時 %20d\n",i+1, 
			top_list[i], savedata[1], day, hour, savedata[0] );
	}
	msg += "\n元素魔法等級排行榜前 " + size_list + " 名:\n";
	msg += sprintf(" 名次  玩家姓名           等級          上線時間                    魔法等級\n");
	for(i=0; i<size_list; i++) {
