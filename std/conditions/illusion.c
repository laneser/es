...
void effect(object player)
{
    ...
    tell_object(player, set_color("你冷的不住顫抖...\n", "HIB", player));
    tell_object(player, set_color("你熱的汗出如漿...\n", "HIY", player));
    cond_data[1]--;
    ...
    tell_room(environment(player), player->query("c_name") + "不住地顫抖，似乎在忍受著極大的痛苦...\n", player);
}

void remove_effect(object player)
{
    tell_object(player, set_color("你的腦子終於從幻覺中清醒過來...\n", "HIC", player));
...
varargs void apply_effect(object player, int duration, int damage)
{
    ...
    if (!cond_data)
    {
      	tell_object(player, set_color("你覺得自己走進了一個奇妙的地方，有著火焰做的地板和冰做的牆...\n", "HIG", player));
	...
	tell_object(player, set_color("你的感覺越來越強烈，彷佛像真的一樣...\n", "HIG", player));
	player->set("conditions/" + EFFECT_NAME, cond_data);
    }
}
