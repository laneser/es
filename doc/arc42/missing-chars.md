...
全樹 `*.c` 原有 945 處 `□`，已依上下文補回 728 處（見 git log）。
這份是**剩下的 217 處**，給熟悉遊戲設定的人逐一認字用。
重新產生統計：`grep -rn '□' --include='*.c' .`

**這個統計只涵蓋 `*.c`。** `doc/help/` 底下的說明文件沒有副檔名，從來不在計數內，
所以上面那 945／217 的數字不含它們。該目錄的 15 處缺字已另外補完：
...
| 類別 | 處數 | 檔數 | 怎麼辦 |
|---|---|---|---|
| C 猜不出原字 | 144 | 75 | 靠上下文認，認出來直接改 |
| B 彩色中文殘骸 | 57 | 7 | 要同時補回漢字與被吃掉的 ESC |
| A 疑似框線／項目符號 | 16 | 3 | 先確認是缺字還是原本就長這樣 |
...
但五個螃蟹檔的線上原圖橫線是 `─`（U+2500），可見**橫線是轉碼時整批換成 `—` 的**，
不是原作的寫法。同一批還有 `︽`→`《`、`︳`→`｜`：這些字元兩種編碼都收得進去，
不會變成 `□`，**光看別處一律是 `《` 認不出這裡原本是 `︽`**。
所以「別處都是 X」既不能證明這裡是 X，也要留意 `□` 旁邊的字元可能本身就已經被換過。
全樹目前只有那五個螃蟹檔用 `─`。
