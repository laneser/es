...
//         "/d/healer/acupuncture/fatal.c"->damage(me,target,tar_point,part_name) ;
           if(target == me)
             {
           ...
           if(target->query_level()<5)
             {
                ...
                tell_room(environment(me),me->query("c_name")+"的手突然伸到"+
                      target->query("c_name")+"面前,正當千鈞一髮之際,\n他的手莫名其妙的僵住, 頭上同時多了好幾個包。\n",me);
                return 1;
             }  
