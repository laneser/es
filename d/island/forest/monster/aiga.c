#include "../tsunami.h"

inherit MONSTER;

void create()
{
    ::create();
    set_level(18);
    set_name( "Princess Aiga", "阿重霞" );
    add( "id",({"aiga","princess"}) );
    set_short( "阿重霞公主");
    set_long(@C_LONG
阿重霞是樹雷王家的第一公主。發現你在偷看，於是對你深情一笑．．．！雖然她
長得十分可愛，但卻十分任性。千萬不要惹她生氣哦，她生氣起來可是很可怕的，你
可別被她的外表迷惑了哦！她不知為何，臉上帶有一絲絲的憂鬱，到底是什麼事情令
她如此哀傷(sadness)呢？
C_LONG
            );
    set("gender","female");
    set("unit","位");
    set("class","scholar");
    set("race","daemon");
    set("age",18);
    set_perm_stat("str",21);
    set_perm_stat("dex",16);
    set("alignment",1000);
    set("weight",400);
    set("wealth/gold",200);
    set("hit_points",550);
    set("max_hp",550);
    set("special_defense",(["evil":55,"fire":50,"energy":80,"electric":70,
                            "cold":0,"none":30]) );
    set("aim_difficulty",
    (["critical":90,"weakest":10,"ganglion":10,"vascular":70]) );
    
    set_skill("longblade",80);
    set_skill("parry",70);
    set_skill("tactic",80);
    set_skill("dodge",70);
    set("tactic","berserk");
    set_natural_armor(76,35);
    set_natural_weapon(35,10,15);
    set("tactic_func","my_tactic");
    set("inquiry",([
         "sadness":"@@ask_sadness"]) );
    set("exp_reward",22000);   

    wield_weapon(TWEP"key1");
    equip_armor(TOBJ"veil");
    equip_armor(TOBJ"cloak");
}

int my_tactic()
{
    object victim;
    if( random(100) > 15 || !victim=query_attacker()  ) return 0;
    if( victim->query("stop_attack") ) return 0; 
     tell_object(victim,
      "\n阿重霞召喚一堆小木柱在你的周圍佈下層層的結界，限制你的行動....\n\n"
          );
     tell_room( environment(victim),
      "\n阿重霞召喚一堆小木柱在四周圍佈下結界，限制 "+victim->query("c_name")+" 的行動.\n\n"
      ,victim  );
     victim->block_attack(10);
     victim->set_temp("msg_stop_attack",
     "( 你被層層結界限制住，無法出手攻擊！)\n" );     
      return 1;
}

void ask_sadness(object who)
{
    if( !(who->query_temp("luoko_quest")) ){
    tell_object(who,@ALONG
唉..........！最近我在練劍時總有股不祥的感覺，覺得......覺得手中這把主鑰似乎
越來越不順手了，心頭也不斷的悸動，好像有什麼大事要發生了一樣，難道守護靈龍皇
的力量要消失了嗎？請你幫我調查一下好不好？我只希望那維護民族命脈的守護靈不要
出事才好..........!
ALONG
         );
     who->set_temp("luoko_quest",1);
	return ;
    }
    tell_object(who,
         "你.........你到底要不要幫人家呀？？要的話就快嘛，人家很急耶！\n");
	return ;
}

void init()
{
    ::init();
    add_action("do_report","report");
}

int do_report(string str)
{
    string *name;
    name = this_player()->query("id");
    
    if( !str || str != "fine") {
    write(@FINE
阿重霞睜大眼睛，期待著你對她說：龍皇沒事了(fine)！！
FINE
         );
    return 1;
    }
    if( !this_player()->query_temp("seed_ok")==1 )
    return notify_fail(
    "阿重霞破口罵道：哼！你想騙我？我知道你根本就沒真心幫我，不裡你了！\n");
    
  if( (int)this_player()->query_temp("luoky_killer")==1 ){
     write("雖然我很感謝你為我解憂，但你殺了我妹妹的寵物，還是不可饒恕，死吧\n");
     command( "kill "+name[0] );
     return 1;
     } 
    
    this_player()->delete_temp("seed_ok");
    if( (int)this_player()->query_quest_level("save_tree") < 1 ) {
    write("阿重霞很高興的說：太好了，真謝謝你，我又可以專心練劍了！\n");
    this_player()->finish_quest("save_tree",1);
    write("你已經完成了這個任務，你得到1000點經驗值\n");
    this_player()->gain_experience(1000);
    return 1;
    }
    write("阿重霞公主微笑的說道：謝謝你再次幫我解除心中的憂慮，謝謝！\n");
    return 1;
}       

void die()
{
    object killer;
    killer = query("last_attacker");
    killer->set_temp("get_key",1);
    ::die();
    return;
}
    
