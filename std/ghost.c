...
//  Add action_points, by Iris
	...
	tell_room( environment(),
		(string)body->query("c_name") + "的影子閃了幾閃，變得和四周的景物一樣清晰。\n" ,
		({ body }) );
	body->save_me();
...
varargs int move_player(mixed dest, string message, string dir)
{
	...
	if(!query("invisible")) {
		if(message == 0 || message == "") {
			tell_room(prev,
				query("c_name") + "的鬼魂飄向" + to_chinese(dir) + "方。\n" ,
				({ this_object() }));
			tell_room( environment(),
				query("c_name") + "的鬼魂飄了過來。\n" ,
				({ this_object() }));
		} else {
                        if( pointerp(message) ) {
                                tell_room(prev, ({
...
int quit(string str) {
	...
	say(query("c_name") + " 離開 ES 回到殘酷的現實了.\n");
...
string query_short()
{
	if(!link) return "一個模糊的白影";
	if(!link->query("c_name"))  return "一個模糊的白影";

	if(interactive(this_object()))
		return link->query("c_name") + "的鬼魂";
	else return link->query("c_name") + "的鬼魂 [斷線中...]";
}
...
void setup_ghost() {
	...
	set("long", sprintf("這個模糊的影子看起來像是%s的鬼魂。\n",link->query("c_name")));
}
