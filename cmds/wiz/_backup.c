...
int cmd_backup(string user)
{
	...
	if( !user || user == uid )
		write(can_read_chinese()? "備份資料... Ok.\n":
			"Backup save data... Ok.\n");
	else
		write(can_read_chinese()? "備份 "+user+" 的資料... Ok.\n":
			"Backup "+user+"'s save data... Ok.\n");
	if( wizardp(this_player()) ) {
		write(can_read_chinese()? "備份資料存於:\n": "Save data to:\n");
...
將你的資料備份起來，假使不幸遇到 crash 或是資料錯誤還能救得回來。
假如你想備份的是現在的狀態, 請先下 save 指令。
