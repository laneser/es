...
int connect_room(string arg)
{
	...
	tell_room( here, ({
		...
		this_player()->query("c_cap_name") + "用巫師魔杖打開一條通往異次元的通道！\n"}),
		this_player() );
	write("Ok.\n");
	return 1;
}
...
int make_room(string arg)
{
	...
	room += "\nvoid create()\n{\n";
	room += "\t::create();\n";
	room += "\tset_short(\"A empty room\", \"空房間\");\n";
	room += "\tset_long(\n";
	room += "\t\t\"This is a simple room created by roommaker.\\n\",\n";
	room += "\t\t\"這是一個用房間編輯器造出來的空房間。\\n\"\n";
	room += "\t);\n";
	room += "}\n";
...
int save_room(string arg)
{
	...
	if( !env ) return notify_fail( "你現在並沒有在任何房間內。\n");
...
//		room += "\ninherit ROOM;\n";
	...
	if( file_size(filename)>-1 ) {
		if ( sscanf(filename,"%s.c",backfile)!=1 ) 
		  return notify_fail("檔案名稱錯誤\n");
		backfile += ".bak";
		if ( !cp (filename,backfile) ) 
		   return notify_fail("原檔備份錯誤!\n");
		if ( !rm(filename) )
		   return notify_fail("舊檔刪除錯誤!\n");  
	}
        filename=filename;
...
void save_room_file(string yn, string filename, string room, int chinese_mode )
{
	if( !yn || yn=="" ) {
		write( chinese_mode?
			"要不要取代 " + filename + " ? (輸入 \"y\" 表示要，\"n\" 表示不要) ":
...
這根魔杖提供以下幾個指令:
    mkroom   - 製造出一個簡單房間的模型。
    saveroom - 將你現在所在的房間嘗試以 LPC 的程式寫成一個檔案。
    rset     - 設定這個房間的 properties。
    radd     - 增加這個房間的 properties。
    connect  - 接通一個通往其他房間的出口。
C_HELP_STAFF
	);
