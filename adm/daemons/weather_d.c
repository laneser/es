	...
	if( random(100) < 30 - 70 * ((int)usr[0]->query("alignment") + 400) / 4600 ) {
		if( environment(usr[0])->query("outside") ) {
			tell_room( environment(usr[0]), 
				"\n你看到天空中一道電光，緊接著「轟隆」一道雷電挾萬鈞之勢由天而降，打在\n"
				"\n你面前的" + usr[0]->query("c_name") + "頭上！\n",
				usr[0] );
			tell_object( usr[0], 
	...
	/* give more chance to aviod the thunder.     -EGA */
		if( environment(usr[0])->query("outside") ) {
			tell_room( environment(usr[0]), 
				...
				usr[0] );
			tell_object( usr[0], 
				"\n突然，一道閃電由天而降，打在你的身旁，巨大的響聲在你耳中迴響不已，過\n"
				"\n了好一陣子才慢慢散去...\n");

			for( i=0; i<sizeof(usr); i++ )
				tell_object( usr[i], 
					"\n天空中一個聲音說道: 多行不義，必遭天譴，今日警示" + 
					STATS_D->alignment_string(usr[0]->query("alignment"))+"之徒: " 
					+ usr[0]->query("c_name") + "\n         ，若不悔改，他日必報！\n\n");
...
varargs void call_thunder(string who)
{
	...
	for( i=0; i<sizeof(usr); i++ ) {
		message( "weather", 
			"\n你看到天空中電光閃閃，夾著一陣陣沈悶的雷聲 .....\n\n"
			,usr[i] );
		if ( (string)usr[i]->query("race") == "beholder" ) {
			tell_object(usr[i],"哇 !! 又打雷了 !! 趕快找個地方躲躲　\n");
			tell_room(environment(usr[i]),sprintf("%s聽到雷聲　嚇得全身都轉成綠色　\n",usr[i]->query("c_name")),usr[i]);
		}
		else if ( (string)usr[i]->query("race") == "shapeshifter" ) {
			usr[i]->drop_eq();
			tell_room(environment(usr[i]),sprintf("%s嚇了一大跳　身上的裝備叮叮噹噹的掉了一地　\n",usr[i]->query("c_name")),usr[i]);
		}
	}
...
//	if( j > 23 ) j = 0;
	...
	if( j > 12 ) j -= 12;
	return sprintf("王國曆 %d 年，%s %d 日，%s %d 時 %d 分",current_year,
